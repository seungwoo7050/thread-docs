...
- 상위 디렉터리가 현재 사용자 소유의 0700이 아니면 왜 파일 자체가 0600이어도 거부해야 합니까?
  - 모범답변: 파일 이름을 교체하거나 다른 경로로 바꾸는 권한은 상위 디렉터리가 결정합니다. 다른 사용자가 디렉터리에 접근·수정할 수 있으면 파일 모드가 0600이어도 open 전후에 경로를 바꾸는 공격 표면이 생기므로, 원본은 현재 UID 소유이고 group/other 비트가 없는 디렉터리만 허용합니다.
- 심볼릭 링크만 막으면 하드링크 공격도 막을 수 있습니까?
  - 모범답변: 아닙니다. `O_NOFOLLOW`는 마지막 경로의 symlink 추적을 막지만 같은 inode를 가리키는 하드링크는 일반 파일로 열립니다. 그래서 원본은 열린 descriptor의 `st_nlink == 1`도 확인해 다른 이름을 통한 공유·교체 가능성을 거부합니다.
- `stat(path)`와 `fstat(fd)` 사이의 차이는 무엇입니까?
  - 모범답변: `stat(path)`는 호출 시점에 그 경로가 가리키는 대상을 검사하므로 이후 open 전까지 바뀔 수 있습니다. `fstat(fd)`는 실제로 열린 객체를 검사하므로 읽을 바이트와 소유권·종류·모드 판단이 같은 객체에 묶입니다.
- FIFO나 장치 파일을 일반 파일처럼 읽으면 어떤 장애가 생길 수 있으며 `O_NONBLOCK`은 어떤 역할을 합니까?
  - 모범답변: FIFO는 writer를 기다리며 open이나 read가 무기한 멈출 수 있고 장치 파일은 예상하지 않은 I/O를 수행할 수 있습니다. `O_NONBLOCK`은 특히 FIFO open의 대기를 막고, 이어지는 `fstat`의 일반 파일 검사가 이런 객체를 확정적으로 거부하게 합니다.
- 파일 크기 제한과 비밀번호 형식 제한은 각각 보안과 운영 측면에서 어떤 이점이 있습니까?
  - 모범답변: bounded read는 거대한 파일로 인한 메모리·시간 소비를 제한합니다. 24~128자와 허용 문자 집합 검사는 빈 값, 여러 줄, 제어 문자처럼 shell·SQL·설정 파일 경계에서 해석이 달라질 값을 일찍 거부해 모든 도구가 같은 계약을 사용하게 합니다.
- 서로 다른 secret 이름이 같은 canonical path를 가리키는 것을 왜 거부해야 합니까?
  - 모범답변: 한 파일을 두 자격증명의 원본으로 공유하면 하나만 회전하거나 롤백한다는 가정이 깨지고 서로 달라야 할 권한 경계도 합쳐집니다. 원본은 Compose에서 얻은 모든 경로를 실제 경로로 canonicalize한 뒤 중복을 거부합니다.
...
def read_private_secret(
    ...
    *,
    ...
    if max_bytes < 1 or not (1 <= min_length <= max_length <= max_bytes):
        raise SecretFileError("invalid secret size limits")
    if not hasattr(os, "O_NOFOLLOW"):
        raise SecretFileError("O_NOFOLLOW is required by the secret file contract")

    expanded = path.expanduser()
    descriptor = -1
    try:
        parent_info = os.lstat(expanded.parent)
        if (
            ...
            raise SecretFileError("secret parent is not a private owned directory")
        ...
        if not stat.S_ISREG(info.st_mode):
            raise SecretFileError("secret is not a regular file")
        if info.st_uid != os.getuid():
            raise SecretFileError("secret file has an unexpected owner")
        if stat.S_IMODE(info.st_mode) != 0o600:
            raise SecretFileError("secret file mode must be exactly 0600")
        if info.st_nlink != 1:
            raise SecretFileError("secret file must have exactly one hard link")

        # 상한보다 한 바이트만 더 읽어 전체 파일을 적재하지 않고 초과를 감지한다.
        raw = os.read(descriptor, max_bytes + 1)
        if len(raw) > max_bytes:
            raise SecretFileError("secret file exceeds the size limit")
        try:
            value = raw.decode("utf-8")
        except UnicodeDecodeError as error:
            raise SecretFileError("secret file is not valid UTF-8") from error
    except SecretFileError:
        raise
    except OSError as error:
        raise SecretFileError("secret file could not be read safely") from error
    finally:
        if descriptor >= 0:
            os.close(descriptor)
    ...
    if not min_length <= len(value) <= max_length or allowed.fullmatch(value) is None:
        raise SecretFileError("secret value does not satisfy the format contract")
    return value
```
...
1. 경로 기반 선행 검사와 descriptor 기반 사후 검증을 함께 둔 이유
   - 선행 검사는 상위 디렉터리의 소유권과 접근 권한을 확인해 경로 namespace 자체를 제한합니다. 그래도 검사와 open 사이 교체 가능성이 있으므로, 실제 읽는 객체의 종류·UID·모드·링크 수는 열린 descriptor에서 다시 확인해야 합니다.
2. symlink와 하드링크가 만드는 위협의 차이
   - symlink는 경로 해석을 다른 대상에 위임하므로 `O_NOFOLLOW`로 마지막 링크 추적을 막습니다. 하드링크는 같은 inode의 또 다른 정상 이름이라 그 flag로 구분되지 않으므로 링크 수 1을 요구해 외부 별칭을 거부합니다.
3. FIFO에 대한 `O_NONBLOCK`과 크기 제한의 역할
   - `O_NONBLOCK`은 공격자가 만든 FIFO를 열 때 writer를 무한히 기다리는 장애를 막고, `fstat`가 곧 일반 파일이 아님을 판정하게 합니다. 크기 제한은 정상 파일이라도 과도한 입력을 한정된 read로 거부합니다.
4. 정확한 0600·현재 UID·private parent를 동시에 요구한 이유
   - 0600은 파일 읽기·쓰기 주체를 소유자로 제한하고, 현재 UID 검사는 그 소유자가 실행 주체인지 확인합니다. private parent는 다른 사용자가 파일 이름을 관찰·교체하는 것을 막으므로 세 조건이 서로 다른 경계를 보호합니다.
5. canonical path 중복을 회전·백업 도구에서 거부해야 하는 이유
   - 표면 경로가 달라도 같은 inode나 실제 경로를 가리키면 독립 secret이라는 전제가 거짓이 됩니다. 회전 순서, 변경 전후 비교, 롤백이 서로 덮어쓰지 않도록 실제 경로 기준의 유일성이 필요합니다.
...
- 스레드 잠금만으로 충분하지 않은 이유는 무엇입니까?
  - 모범답변: 시작·백업·복원·회전은 서로 다른 CLI 프로세스로 동시에 실행될 수 있습니다. 한 프로세스 안의 mutex는 다른 프로세스에 보이지 않으므로, 모든 도구가 같은 파일 descriptor 기반 `flock` 계약을 사용해야 합니다.
- 잠금 파일 자체가 symlink라면 어떤 공격이 가능합니까?
  - 모범답변: 공격자가 lock 경로를 임의 파일로 연결해 다른 inode에 잠금을 잡게 하거나, 생성·권한 변경이 예상 밖 대상에 적용되게 만들 수 있습니다. 원본은 `O_NOFOLLOW`로 열고 `fstat`으로 일반 파일과 소유자를 확인합니다.
- blocking lock과 non-blocking lock 중 운영 명령에는 무엇이 더 적합합니까?
  - 모범답변: 이 프로젝트는 non-blocking이 적합합니다. 이미 다른 관리 작업이 진행 중임을 즉시 알려 사용자가 중복 명령을 인지하게 하고, 알 수 없는 시간 동안 대기하면서 CI timeout이나 자동화 교착을 만들지 않습니다.
- 프로세스가 비정상 종료되면 advisory file lock은 어떻게 해제됩니까?
  - 모범답변: 커널이 프로세스의 열린 descriptor를 닫으면서 `flock`도 해제합니다. lock 파일 자체는 남아도 소유 중이라는 뜻이 아니므로 존재 여부가 아니라 실제 lock 획득 결과로 busy 상태를 판단합니다.
- 같은 프로젝트의 읽기 작업도 모두 배타 잠금을 잡아야 합니까?
  - 모범답변: 일관된 여러 자원의 snapshot이 필요한 백업처럼 읽기처럼 보여도 변경 작업과 겹치면 안 되는 연산은 배타 잠금이 필요합니다. 단일 상태 조회처럼 불일치를 허용할 수 있는 진단은 잠금을 생략할 수 있지만, 그 계약을 명확히 해야 합니다.
- 잠금을 획득한 뒤 오래 걸리는 subprocess가 hang하면 어떤 문제가 생기며 어떻게 제한합니까?
  - 모범답변: 해당 프로젝트의 모든 관리 작업이 계속 거부되고 일부 서비스가 중지된 임계 상태가 길어질 수 있습니다. 원본은 모든 Compose·Docker 호출에 유한한 timeout을 두고 예외 경로에서도 context manager가 descriptor를 닫도록 합니다.
...
def project_operation_lock(
    ...
    *,
    ...
    if re.fullmatch(r"[a-z0-9][a-z0-9_-]{2,62}", project_name) is None:
        raise ValueError("invalid project name")
    if not hasattr(os, "O_NOFOLLOW"):
        raise OperationBusyError("O_NOFOLLOW is required by the lock contract")

    lock_directory = root / f"container-stack-operation-locks-{os.getuid()}"
    try:
        lock_directory.mkdir(mode=0o700)
    except FileExistsError:
        pass
    except OSError as error:
        raise OperationBusyError("lock directory could not be created") from error
    ...
    try:
        ...
        if (
            ...
            raise OperationBusyError("lock directory is not private and owned")
        ...
        raise OperationBusyError("lock directory could not be opened safely") from error
    ...
    try:
        ...
        if not stat.S_ISREG(lock_info.st_mode) or lock_info.st_uid != os.getuid():
            raise OperationBusyError("lock file is not a regular owned file")
        try:
            fcntl.flock(lock_fd, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as error:
            raise OperationBusyError(
                f"another operation is active for project {project_name}"
            ) from error
        try:
            yield
        finally:
        ...
        raise OperationBusyError("project lock could not be acquired safely") from error
    finally:
...
1. advisory lock이 동작하려면 모든 관리 도구가 같은 계약을 따라야 하는 이유
   - advisory lock은 커널이 다른 프로세스의 접근을 강제로 막는 장치가 아닙니다. 시작·백업·복원·회전 도구가 모두 같은 프로젝트 key와 lock 위치를 사용해 임계 구역에 들어가기 전에 자발적으로 획득해야 직렬화가 성립합니다.
2. lock file 존재와 실제 잠금 소유가 다른 이유
   - 파일은 프로세스가 끝난 뒤에도 재사용을 위해 남을 수 있지만 `flock` 소유권은 열린 descriptor에 붙습니다. 따라서 파일 존재 검사는 stale 상태를 busy로 오인하며, non-blocking lock 호출 결과가 실제 소유 여부입니다.
3. 비차단 획득을 선택한 운영 UX상의 이유
   - 충돌을 즉시 명시적 오류로 보여 주어 사용자가 진행 중인 작업을 확인하거나 재시도 시점을 결정하게 합니다. 무기한 대기하면 어느 작업이 상태를 점유하는지 숨기고 자동화 timeout만 키울 수 있습니다.
4. 프로젝트 이름 해시가 해결하는 경로 조작·길이·문자 문제
   - 고정 길이 hex digest는 `/`, `..`, 지나치게 긴 이름 같은 경로 의미를 제거하고 파일시스템 이름 제한 안에서 안정적으로 같은 key를 만듭니다. 원래 이름을 lock 파일명에 직접 노출하지 않는 부수 효과도 있습니다.
5. 잠금과 subprocess timeout을 함께 설계해야 하는 이유
   - 잠금만 두면 hang한 외부 명령이 프로젝트 임계 구역을 영구 점유할 수 있습니다. 각 외부 호출의 timeout과 예외 시 descriptor 정리가 함께 있어야 배타성은 유지하면서 장애 후 다른 작업이 다시 진입할 수 있습니다.
...
- 초기화에 비밀번호가 필요한데 runtime에는 필요하지 않다면 어떤 전달 경로가 적절합니까?
  - 모범답변: 원본은 검증한 호스트 secret을 one-off bootstrap 컨테이너의 표준 입력으로 전달합니다. 컨테이너 안에서는 0600 임시 client option file로만 사용하고 trap에서 지운 뒤, 장기 실행 프로세스에는 secret mount나 비밀번호 환경 변수를 남기지 않습니다.
- 환경 변수 이름만 검사하면 충분합니까? 임의 이름에 실제 secret 값이 들어간 경우는 어떻게 찾습니까?
  - 모범답변: 충분하지 않습니다. 금지된 표준 이름을 검사하면 실수로 남긴 변수 계약을 잡을 수 있지만, 다른 이름에 값을 복사한 노출은 놓칩니다. 그래서 원본은 inspect 환경과 `/proc` 환경, 프로세스 인자에서 실제 현재·이전 secret 값도 검색합니다.
- `/run/secrets` 디렉터리 자체와 그 하위 mount를 모두 검사해야 하는 이유는 무엇입니까?
  - 모범답변: 전체 secret 디렉터리를 mount할 수도 있고 개별 secret을 `/run/secrets/name`에 mount할 수도 있기 때문입니다. 경로 경계를 기준으로 정확히 두 경우를 잡되 `/run/secrets-old` 같은 별도 경로는 오탐하지 않아야 합니다.
- `docker inspect` 결과가 예상 구조가 아니거나 컨테이너를 하나로 식별하지 못하면 왜 실패해야 합니까?
  - 모범답변: 검증 대상이 없거나 여러 개인데 정상으로 통과시키면 실제 실행 인스턴스의 노출을 확인했다는 보장이 사라집니다. 원본은 service별 container ID가 정확히 하나인지와 inspect가 단일 객체 목록인지 확인하고 아니면 fail closed 합니다.
- DB client 비밀번호를 command argument로 넘기면 inspect 외에 어떤 경로로 노출될 수 있습니까?
  - 모범답변: 같은 호스트의 process listing, `docker top`, `/proc/<pid>/cmdline`, shell history나 오류 로그에 나타날 수 있습니다. 원본은 비밀번호를 stdin으로 받아 임시 option file에 쓰고 CLI argv에는 파일 경로만 전달합니다.
...
def runtime_secret_violations(
    ...
    *,
    ...
    findings: list[str] = []
    searched_secrets = tuple(value for value in secret_values if value)

    def exposed(text: str) -> bool:
        return any(value in text for value in searched_secrets)

    for service in sorted(containers):
        container = containers[service]
        if not isinstance(container, Mapping):
            findings.append(f"{service}: invalid container structure")
            continue

        mounts = container.get("Mounts")
        if not isinstance(mounts, (list, tuple)):
            findings.append(f"{service}: invalid mounts structure")
        else:
            for index, mount in enumerate(mounts):
                if not isinstance(mount, Mapping):
                    findings.append(f"{service}: invalid mount structure at {index}")
                    continue
                source = mount.get("Source", "")
                destination = mount.get("Destination", "")
                if not isinstance(source, str) or not isinstance(destination, str):
                    findings.append(f"{service}: invalid mount path at {index}")
                    continue
                if destination == "/run/secrets" or destination.startswith("/run/secrets/"):
                    findings.append(f"{service}: runtime secret mount at {index}")
                if exposed(source):
                    findings.append(f"{service}: secret value exposed in mount source at {index}")
                if exposed(destination):
                    findings.append(f"{service}: secret value exposed in mount destination at {index}")

        config = container.get("Config")
        if not isinstance(config, Mapping):
            findings.append(f"{service}: invalid config structure")
            continue
        environment = config.get("Env")
        if not isinstance(environment, (list, tuple)):
            findings.append(f"{service}: invalid environment structure")
        else:
            for index, entry in enumerate(environment):
                if not isinstance(entry, str) or "=" not in entry:
                    findings.append(f"{service}: invalid environment entry at {index}")
                    continue
                name, value = entry.split("=", 1)
                if name in forbidden_env_names:
                    findings.append(f"{service}: forbidden environment name {name}")
                if exposed(value):
                    findings.append(f"{service}: secret value exposed in environment at {index}")
        ...
        for label, values in string_fields:
            if values is None:
                continue
            if not isinstance(values, (list, tuple)) or not all(
                isinstance(value, str) for value in values
            ):
                findings.append(f"{service}: invalid {label} structure")
                continue
            for index, value in enumerate(values):
                if exposed(value):
                    findings.append(f"{service}: secret value exposed in {label} at {index}")

        process_path = container.get("Path", "")
        if not isinstance(process_path, str):
            findings.append(f"{service}: invalid process path structure")
        elif exposed(process_path):
            findings.append(f"{service}: secret value exposed in process path")
...
1. 저장 시 보안과 runtime 비노출이 별도 요구사항인 이유
   - 호스트 파일이 0600이어도 컨테이너에 mount되거나 환경·argv로 복사되면 더 많은 프로세스와 진단 인터페이스에서 읽힐 수 있습니다. 저장 매체의 접근 통제와 실행 중 전파 최소화는 서로 다른 검증 대상입니다.
2. secret 이름 검사와 실제 값 검사 둘 다 필요한 이유
   - 이름 검사는 알려진 금지 변수가 값이 비어 있거나 예상 밖 형식이어도 정책 위반을 잡습니다. 실제 값 검사는 임의 이름, command, process 환경으로 복사된 노출을 찾으므로 두 검사가 상호 보완적입니다.
3. one-off bootstrap stdin과 장기 실행 mount의 trade-off
   - stdin은 초기화 프로세스 수명에 전달 범위를 묶고 종료 뒤 mount를 남기지 않는 장점이 있습니다. 재시작 때마다 비밀이 필요한 애플리케이션에는 mount가 운영상 편할 수 있지만, 이 프로젝트는 초기화 후 설정·DB에 상태가 남으므로 장기 mount의 공격 표면을 피했습니다.
4. 임시 option file을 사용하고 command argument를 피한 이유
   - DB client가 인증 파일을 요구하는 짧은 구간에만 0600 파일을 만들고 trap으로 삭제하면 비밀번호가 argv와 process listing에 나타나지 않습니다. 표준 입력은 비밀번호와 SQL payload를 bootstrap 경계까지 안전하게 운반합니다.
5. inspect 구조 오류를 fail-open으로 처리하지 않은 이유
   - 구조가 다르면 mount나 환경을 실제로 검사하지 못했다는 뜻입니다. 이를 위반 없음으로 처리하면 검증 성공의 의미가 사라지므로, 원본은 대상 식별과 자료 형태가 예상과 다를 때 별도 오류로 중단합니다.
...
- root 비밀번호 변경 직후 실패하면 현재 비밀번호와 새 비밀번호 중 무엇으로 DB에 접속해야 합니까?
  - 모범답변: 어느 쪽이라고 가정하지 않고 둘 다 실제 인증으로 탐색해야 합니다. 원본 rollback은 새 root와 기존 root 후보를 중복 없이 차례로 `SELECT 1`에 사용해 현재 동작하는 자격증명을 찾습니다.
- 호스트 secret 파일을 먼저 바꾸는 것과 마지막에 바꾸는 것의 trade-off는 무엇입니까?
  - 모범답변: 먼저 바꾸면 이후 도구가 새 값을 읽지만 DB·WordPress가 아직 옛 값인 불일치 창이 생깁니다. 마지막에 바꾸면 실행 상태를 먼저 전환할 수 있지만 그 사이 재시작은 옛 파일을 읽습니다. 원본은 서비스를 제한하고 내부 상태를 바꾼 뒤 host 파일을 원자 교체하고 곧바로 강제 재생성·검증하며, 전체를 잠금과 rollback으로 감쌉니다.
- rollback 자체의 일부 단계가 실패하면 결과를 어떻게 보고해야 합니까?
  - 모범답변: 첫 보상 오류에서 중단하지 않고 DB 계정, 설정, 사용자, host 파일, 최종 재기동을 가능한 만큼 계속 복구합니다. 원래 회전 오류를 예외 원인으로 유지하고 각 보상 오류와 최종 recovered 여부를 함께 보고해야 운영자가 남은 상태를 판단할 수 있습니다.
- rollback 중 SIGTERM이 한 번 더 오면 즉시 중단해야 합니까?
  - 모범답변: 원본은 즉시 중단하지 않습니다. 이미 일관성을 회복하는 중이므로 추가 SIGINT·SIGTERM은 deferred flag로 기록하고 rollback과 검증을 끝낸 뒤 결과 메시지에 반영합니다. 강제 SIGKILL은 지연할 수 없으므로 재실행 가능한 보상 절차도 필요합니다.
- 새 secret 파일 세트가 기존 값과 일부 같거나 파일 경로를 공유하면 왜 거부해야 합니까?
  - 모범답변: 일부 값이 같으면 이전 자격증명 거부 검증과 회전 완료 판단이 모호해집니다. 서로 다른 secret 이름이 같은 canonical path를 공유하면 원자 교체와 rollback이 다른 항목을 덮어쓸 수 있으므로, 원본은 모든 새 값의 변경과 현재 source path의 유일성을 side effect 전에 확인합니다.
- 최종 검증에서 "새 비밀번호가 동작한다" 외에 무엇을 확인해야 합니까?
  - 모범답변: 이전 root·애플리케이션·WordPress 사용자 비밀번호가 모두 거부되는지, `wp-config.php`가 새 DB 비밀번호인지, host secret 파일이 새 값인지 확인해야 합니다. 또한 서비스 강제 재생성 뒤 정상 접근과 runtime mount·환경·프로세스 인자에 현재나 이전 secret이 남지 않았는지도 검증합니다.
...
def execute_rotation(
    ...
    *,
    ...
    if len(set(names)) != len(names):
        raise ValueError("rotation step names must be unique")

    compensation_candidates: list[RotationStep] = []
    try:
        for step in steps:
            # apply가 상태를 바꾼 뒤 예외를 낼 수 있으므로 호출 직전에 후보로 기록한다.
            # 따라서 compensate는 적용 전 상태에서도 안전한 idempotent callback이어야 한다.
            compensation_candidates.append(step)
            step.apply()
            step.verify_applied()
        final_verify()
        return
    except BaseException as original_error:
        rollback_errors: list[tuple[str, str, BaseException]] = []
        for step in reversed(compensation_candidates):
            try:
                step.compensate()
            except BaseException as error:
                rollback_errors.append((step.name, "compensate", error))
                # 보상 실패와 별개로 현재 상태를 검증하고 나머지 단계도 계속한다.
            try:
                step.verify_compensated()
            except BaseException as error:
                rollback_errors.append((step.name, "verify", error))

        detail = ", ".join(
            f"{name}:{phase}:{type(error).__name__}"
            for name, phase, error in rollback_errors
        )
        message = "rotation failed and compensation was attempted"
        if detail:
            message += f"; rollback errors=[{detail}]"
        failure = RotationFailed(message)
        # 원래 오류는 체인으로, 개별 보상 오류는 값 노출 없는 구조화 속성으로 보존한다.
        failure.original_error = original_error  # type: ignore[attr-defined]
        failure.rollback_errors = tuple(rollback_errors)  # type: ignore[attr-defined]
...
1. 이 문제를 ACID 트랜잭션이 아니라 saga/보상 트랜잭션으로 본 이유
   - DB 계정, WordPress 사용자 DB, 설정 파일, host 파일, 컨테이너 재생성은 하나의 transaction manager에 참여하지 않습니다. 전역 commit/rollback이 없으므로 각 변경 뒤 확인하고 이미 적용된 단계를 역순 작업으로 되돌리는 saga가 실제 실패 모델에 맞습니다.
2. 단계별 검증과 최종 종단 검증을 모두 둔 이유
   - 단계별 검증은 실패 지점과 보상 대상을 좁히고 다음 단계가 잘못된 전제에서 실행되는 것을 막습니다. 최종 검증은 개별 저장소가 맞아도 서비스 재생성, 상호 인증, 이전 값 거부와 runtime 비노출이 함께 성립하는지 확인합니다.
3. root 자격증명을 현재·새 후보 중 실제 인증으로 찾는 이유
   - root 변경 명령은 비밀번호를 바꾼 뒤 통신 오류나 주입된 실패를 낼 수 있어 예외만으로 적용 여부를 알 수 없습니다. 두 후보를 실제 쿼리로 시험해야 rollback SQL을 실행할 수 있는 현재 자격증명을 결정할 수 있습니다.
4. rollback 중 추가 종료 신호를 지연해야 하는 이유
   - 보상 중 다시 중단되면 여러 저장소가 더 모호한 조합으로 남고 재시도 판단도 어려워집니다. 원본은 추가 신호를 기록만 하고 복구와 최종 검증을 완료해 가능한 한 알려진 상태로 돌아갑니다.
5. 원래 오류와 보상 오류를 함께 보존하면서 재시도 가능성을 유지하는 방법
   - 원래 오류를 예외 chain에 두고 보상 단계별 오류와 최종 recovered 여부를 별도로 집계합니다. 보상 callback과 원자 파일 교체를 멱등하게 만들고, 재시도 시 현재 자격증명을 실제 probe로 다시 찾으면 부분 성공 뒤에도 같은 절차를 안전하게 반복할 수 있습니다.
