...
- `data_`를 빈 문자열에서도 null로 두지 않고 종료 문자 한 바이트를 소유하게 하면 어떤 장점이 있습니까?
  - 모범답변: 프로젝트의 `TextBuffer`처럼 모든 정상 객체에서 `c_str()`이 즉시 유효한 C 문자열을 반환하므로, 생성자·복사·비교·출력에서 null 분기가 필요 없습니다. 일반적으로는 빈 값에도 1바이트 할당이 필요하다는 비용이 있습니다.
- 자기 대입을 별도 분기로 처리하지 않아도 copy-and-swap이 안전한 이유는 무엇입니까?
  - 모범답변: 우변으로 완전한 깊은 복사본을 먼저 만든 뒤 포인터와 길이만 교환하므로, 우변이 자기 자신이어도 복사본은 독립 저장소를 가집니다. 복사가 실패하면 swap에 도달하지 않아 원래 객체도 그대로입니다.
- `buffer += buffer`가 안전하려면 기존 저장소를 언제 해제해야 합니까?
  - 모범답변: 기존 두 구간을 새 저장소에 모두 복사한 뒤에만 기존 저장소를 해제해야 합니다. 그래야 `other.data_`가 `data_`와 같아도 두 번째 복사가 끝나기 전에 원본을 잃지 않습니다.
- `size_ + other.size_ + 1` 계산 자체에서 오버플로가 발생하지 않도록 무엇을 먼저 확인해야 합니까?
  - 모범답변: `other.size_ > max_size_t - size_ - 1`인지 먼저 검사합니다. 이 부등식을 통과한 뒤에만 `joined_size = size_ + other.size_`와 `joined_size + 1`을 계산합니다.
- 복사 생성 중 `new[]`가 실패한 경우와 대입 중 실패한 경우, 원본·대상 객체의 상태는 각각 어떻게 되어야 합니까?
  - 모범답변: 복사 생성 실패 시 새 객체는 성립하지 않지만 원본은 변하지 않아야 합니다. 대입 실패 시에는 임시 복사만 생성되지 않고, 기존 대상의 포인터·길이·내용도 호출 전 상태를 유지해야 합니다.
...
OwnedText::OwnedText() : data_(new char[1]), size_(0)
{
    data_[0] = '\0';
}

OwnedText::OwnedText(const char *text) : data_(0), size_(0)
{
    if (text == 0)
        text = "";
...
{
    OwnedText copy(other); // 복사가 끝나기 전에는 대상 상태를 건드리지 않는다.

    swap(copy);
    return *this;
}

OwnedText &OwnedText::append(const OwnedText &other)
{
    ...
    // other가 *this여도 기존 저장소는 아직 살아 있다.
...
std::size_t OwnedText::size() const
{
    return size_;
}

bool OwnedText::empty() const
{
    return size_ == 0;
}

const char *OwnedText::c_str() const
{
    return data_;
}

char &OwnedText::at(std::size_t index)
{
    if (index >= size_)
...
const char &OwnedText::at(std::size_t index) const
{
    if (index >= size_)
        throw std::out_of_range("text index");
    return data_[index];
...
void OwnedText::swap(OwnedText &other) throw()
{
    char *data = data_;
...
1. 빈 문자열도 한 바이트 저장소를 갖게 한 이유와 null 저장 표현과의 trade-off
   - 모범답변: 이 구현은 빈 값도 `""`를 담은 배열을 소유해 `data_ != 0`, `data_[size_] == '\0'`이라는 하나의 불변식을 유지합니다. 덕분에 `c_str()` 호출자와 내부 복사 코드가 null을 별도 처리하지 않지만, 빈 객체 생성에도 할당 한 번이 필요합니다.
2. copy-and-swap이 자기 대입과 강한 예외 보장을 동시에 단순화하는 방식
   - 모범답변: 우변 복사본 생성이 유일하게 실패할 수 있는 준비 단계이고, 성공 후 swap은 포인터와 길이만 교환해 예외를 던지지 않습니다. 자기 대입도 독립 복사본과 교환한 뒤 임시 객체가 옛 저장소를 정리하므로 별도 분기 없이 안전합니다.
3. `append`에서 기존 저장소를 마지막에 해제해야 자기 결합이 안전한 이유
   - 모범답변: 자기 결합에서는 오른쪽 입력이 기존 `data_`를 가리킵니다. 새 배열에 왼쪽과 오른쪽 내용을 모두 복사하기 전에 기존 배열을 지우면 두 번째 복사가 해제된 메모리를 읽게 되므로, 새 내용 완성 뒤에 해제해야 합니다.
4. 길이 오버플로를 연산 후가 아니라 연산 전에 검사한 식
   - 모범답변: `other.size_ > max_size_t - size_ - 1`이면 종료 문자까지 필요한 바이트 수를 표현할 수 없습니다. unsigned 덧셈은 먼저 수행하면 wrap-around하므로 이 검사를 통과한 뒤 합을 계산합니다.
5. 대입마다 추가 할당이 발생하는 방식과 재사용 최적화 사이의 trade-off
   - 모범답변: copy-and-swap은 기존 용량을 재사용하지 않아 대입마다 새 배열과 임시 객체가 필요하지만 실패 경계가 단순하고 강한 보장이 명확합니다. 용량을 따로 추적해 재사용하면 할당을 줄일 수 있으나 겹침·부분 복사·예외 시 복원 로직이 복잡해집니다.
...
- 기반 클래스 소멸자가 가상이어야 하는 이유는 무엇입니까?
  - 모범답변: pipeline은 파생 객체를 `Transform*`로 소유하고 기반 포인터로 `delete`합니다. 기반 소멸자가 가상이 아니면 파생 소멸자가 호출되지 않아 파생 자원 정리가 보장되지 않습니다.
- `clone()`이 원시 포인터를 반환하는 설계에서 누가 언제 소유권을 취합니까?
  - 모범답변: 프로젝트 계약에서는 `clone()` 호출자가 반환 즉시 독점 소유권을 취합니다. `append`는 clone 성공 후 포인터를 슬롯에 기록하고 `size_`를 증가시키며, 이후 pipeline 소멸자가 삭제합니다.
- 복사 생성 중 네 번째 clone이 실패하면 앞서 성공한 세 객체는 누가 정리해야 합니까?
  - 모범답변: 아직 `TransformPipeline` 객체의 생성이 끝나지 않아 그 소멸자가 자동 호출되지 않으므로, 복사 생성자의 catch 블록이 `[0, size_)`의 세 clone을 직접 삭제한 뒤 예외를 다시 던져야 합니다.
- `append(const Formatter&)`가 원본을 참조만 보관하지 않고 clone하는 이유는 무엇입니까?
  - 모범답변: 외부 formatter가 먼저 소멸하거나 변경되어도 pipeline의 동작과 lifetime이 독립적이어야 하기 때문입니다. clone은 동적 타입을 보존하면서 소유권을 pipeline 내부로 분리합니다.
- 용량 검사를 clone보다 먼저 해야 하는 이유는 무엇입니까?
  - 모범답변: 이미 가득 찬 상태에서는 결과를 저장할 수 없으므로 clone 자체가 불필요합니다. 먼저 검사하면 clone 부작용·비용과 반환 포인터를 즉시 정리해야 하는 추가 소유권 경계를 만들지 않습니다.
- C++11 이후라면 어떤 소유 타입으로 바꾸겠습니까?
  - 모범답변: `clone()`은 `std::unique_ptr<Transform>`을 반환하고 pipeline은 `std::vector<std::unique_ptr<Transform> >`로 소유하게 할 수 있습니다. 부분 생성과 소멸 정리가 RAII로 자동화됩니다.
...
TransformPipeline::TransformPipeline() : steps_(), size_(0)
{
    for (std::size_t i = 0; i < max_steps; ++i)
        steps_[i] = 0;
}

TransformPipeline::TransformPipeline(const TransformPipeline &other)
    : steps_(), size_(0)
{
    for (std::size_t i = 0; i < max_steps; ++i)
        steps_[i] = 0;
    try
    {
        for (std::size_t i = 0; i < other.size_; ++i)
            append(*other.steps_[i]);
    }
    catch (...)
    {
        // 생성이 끝나지 않은 객체에는 소멸자가 호출되지 않는다.
...
TransformPipeline &TransformPipeline::operator=(
    const TransformPipeline &other)
{
    TransformPipeline copy(other);

    swap(copy);
    return *this;
}

void TransformPipeline::append(const Transform &step)
{
    if (size_ == max_steps)
        throw std::length_error("pipeline capacity");

    Transform *copy = step.clone();
    steps_[size_] = copy; // clone 성공 뒤에만 소유 포인터를 공개한다.
    ++size_;
}

std::string TransformPipeline::apply(const std::string &input) const
{
    std::string result(input);

    for (std::size_t i = 0; i < size_; ++i)
        result = steps_[i]->apply(result);
    return result;
}

std::size_t TransformPipeline::size() const
{
    return size_;
}

void TransformPipeline::swap(TransformPipeline &other) throw()
{
    for (std::size_t i = 0; i < max_steps; ++i)
    {
        Transform *step = steps_[i];
...
1. `clone()`이 객체 슬라이싱을 피하면서 값 의미론을 제공하는 방식
   - 모범답변: 가상 `clone()`은 실제 파생 클래스가 자기 복사 생성자를 호출하게 하므로 동적 타입과 파생 상태를 그대로 복제합니다. pipeline은 그 복제본을 독점 소유해 원본과 독립적인 값처럼 복사됩니다.
2. 복사 생성자 자체는 완성된 객체가 아니므로 부분 clone을 직접 정리해야 하는 이유
   - 모범답변: 생성자에서 예외가 나면 해당 객체의 소멸자는 호출되지 않습니다. 따라서 성공한 clone 수를 `size_`로 추적하고 catch에서 그 구간을 직접 삭제해야 부분 구성 자원이 누수되지 않습니다.
3. 대입에서는 copy-and-swap을 사용할 수 있지만 복사 생성에서는 별도 정리가 필요한 이유
   - 모범답변: 대입 시에는 이미 완성된 대상과 지역 복사본이 있어 복사본 소멸자가 옛 자원을 정리할 수 있습니다. 복사 생성 중인 객체는 아직 완성되지 않았으므로 그 소멸자에 정리를 맡길 수 없습니다.
4. `append`가 참조 보관이 아니라 clone 소유를 선택한 lifecycle trade-off
   - 모범답변: clone 소유는 외부 단계의 수명과 변경에서 pipeline을 분리하지만 단계마다 동적 할당과 복사 비용이 듭니다. 참조 보관은 저렴하지만 호출자가 모든 참조 대상의 수명을 더 길게 보장해야 합니다.
5. C++11 이상에서 `std::unique_ptr<Transform>`과 `std::vector`로 단순화할 부분
   - 모범답변: clone 결과와 내부 단계들을 `unique_ptr`로 표현하면 소유권 이전이 타입에 드러나고 부분 생성 정리가 자동화됩니다. `vector`가 크기와 슬롯 관리를 맡아 수동 초기화·삭제·swap 루프도 줄일 수 있습니다.
...
- 기존 pipeline을 먼저 비운 뒤 새 단계를 append하는 구현의 문제는 무엇입니까?
  - 모범답변: 두 번째 이후 명세의 생성이나 clone이 실패하면 옛 pipeline은 이미 사라지고 새 단계 일부만 남습니다. 따라서 호출 전 상태를 보존하지 못하고, 구현에 따라 basic guarantee보다 약한 관찰 가능한 부분 갱신도 노출됩니다.
- creator가 반환한 raw pointer와 `append`가 내부에서 만드는 clone 사이의 두 소유 단계를 어떻게 정리합니까?
  - 모범답변: creator 반환 포인터는 즉시 지역 owner가 소유하고, `append`는 그 객체를 참조해 별도 clone을 candidate에 소유시킵니다. append의 성공·실패와 무관하게 owner는 원본 포인터를 한 번 삭제하고, candidate는 자기 clone만 관리합니다.
- 후보 pipeline이 완성되기 전에 예외가 나면 어떤 소멸자가 무엇을 정리합니까?
  - 모범답변: 현재 반복의 지역 owner가 creator 원본을 정리하고, 이미 append된 clone들은 지역 candidate의 소멸자가 정리합니다. target은 commit 전까지 접근하지 않았으므로 정리 대상도 변경도 없습니다.
- `specifications == 0 && count == 0`과 `specifications == 0 && count != 0`을 왜 다르게 처리합니까?
  - 모범답변: 0개 입력에서는 배열 원소를 읽지 않으므로 null이 유효한 빈 범위 표현이고, 빈 candidate로 교체할 수 있습니다. count가 양수면 null에서 원소를 읽어야 하므로 명세 오류로 먼저 거부해야 합니다.
- 생성 실패, 알 수 없는 명세, clone 실패의 예외 형식을 하나로 바꾸지 않고 그대로 전파하는 장점은 무엇입니까?
  - 모범답변: 호출자가 입력 오류, 지원하지 않는 종류, 자원 부족이나 테스트용 clone 실패를 구분해 진단·복구 정책을 정할 수 있습니다. 이 프로젝트의 replace는 상태 원자성만 책임지고 원인 정보는 보존합니다.
- creator가 `std::unique_ptr`를 반환할 수 있다면 인터페이스를 어떻게 바꾸겠습니까?
  - 모범답변: `create`의 반환형을 `std::unique_ptr<Transform>`으로 바꾸고 지역 변수로 받습니다. 현재 append가 clone 계약이라면 `candidate.append(*created)` 후 자동 소멸시키고, 소유권을 직접 받는 append를 추가한다면 `std::move(created)`로 한 번만 이전할 수 있습니다.
...
{
public:
    explicit TransformOwner(Transform *step) : step_(step)
    {
    }
...
private:
    TransformOwner(const TransformOwner &);
    TransformOwner &operator=(const TransformOwner &);
                     ...
                     std::size_t count)
{
    if ((specifications == 0 && count != 0) ||
        count > TransformPipeline::max_steps)
        throw std::invalid_argument("invalid pipeline specification");

    TransformPipeline candidate;

    for (std::size_t i = 0; i < count; ++i)
    {
        TransformOwner created(creator.create(specifications[i]));

        if (created.get() == 0)
            throw std::invalid_argument("null transform");
        // append는 독립 clone을 소유하고, created는 원본을 정리한다.
        candidate.append(*created.get());
    }
    target.swap(candidate); // 유일한 commit 지점이며 예외를 던지지 않는다.
}
```
...
1. 대상 선삭제 방식이 basic guarantee조차 약화할 수 있는 실패 시나리오
   - 모범답변: target을 비운 뒤 중간 creator나 clone이 실패하면 대상에는 새 단계 일부만 남을 수 있습니다. 객체 자체는 유효하더라도 호출 전 의미를 잃고 부분 작업이 노출되므로 강한 보장은 깨지며, 정리 순서가 잘못되면 누수까지 생길 수 있습니다.
2. candidate의 lifetime이 rollback을 자동화하는 이유
   - 모범답변: 모든 새 clone은 지역 candidate가 소유합니다. commit 전 예외로 스택을 빠져나가면 candidate 소멸자가 현재까지의 clone을 삭제하므로 별도 역연산 없이 새 상태만 폐기됩니다.
3. creator 원본과 pipeline clone이 잠시 동시에 존재하는 비용과 소유권 명확성의 trade-off
   - 모범답변: append 동안 원본과 clone 두 객체가 동시에 있어 메모리와 복사 비용이 증가합니다. 대신 creator 결과는 지역 owner, 실행 단계는 candidate가 각각 소유해 누가 무엇을 삭제하는지가 분명하고 외부 수명과 분리됩니다.
4. 마지막 `swap`을 트랜잭션 commit으로 볼 수 있는 조건
   - 모범답변: candidate가 모든 검증·생성·clone을 끝낸 완전한 상태이고, swap이 대상과 candidate의 소유 포인터·크기만 교환하며 예외를 던지지 않아야 합니다. 그러면 swap 전은 옛 상태, swap 후는 새 상태만 관찰됩니다.
5. 오류를 변환하지 않고 전파할 때 호출자가 얻는 진단 정보
   - 모범답변: 호출자는 잘못된 명세, 미지원 formatter, clone 실패, 메모리 부족을 실제 예외 형식으로 구분할 수 있습니다. replace는 공통 오류로 뭉개지 않고도 모든 경우에 target 보존이라는 동일한 상태 보장을 제공합니다.
...
- 성공 경로에서 먼저 총 할당 또는 clone 횟수를 계측하는 이유는 무엇입니까?
  - 모범답변: 실제 실행 경로가 몇 번의 실패 가능 연산을 거치는지 기준값을 얻어야 첫 지점부터 마지막 지점까지 빠짐없이 sweep할 수 있습니다. 하드코딩한 횟수는 구현 변경 시 경로 일부를 놓치기 쉽습니다.
- 1번째 실패부터 마지막 실패까지 모두 주입해야 하는 이유는 무엇입니까?
  - 모범답변: 첫 실패는 준비 전 경계를, 중간 실패는 부분 구성 정리를, 마지막 실패는 commit 직전 상태를 주로 검증합니다. 한 지점만 검사하면 특정 단계의 누수나 부분 갱신을 놓칠 수 있습니다.
- 단순히 예외가 발생했다는 사실 외에 어떤 상태를 기록해야 합니까?
  - 모범답변: 실제 예외 형식과 실패 시도 번호, live object 또는 live allocation 기준값, source와 target의 크기·출력, 기존 원소 주소, 실패 뒤 정상 재사용 가능성을 함께 확인해야 합니다.
- live object 수와 destroyed count는 각각 어떤 종류의 결함을 찾습니까?
  - 모범답변: live count가 기준으로 돌아오는지는 누수나 과잉 소멸을 찾고, destroyed count는 기대한 rollback 객체와 파생 소멸자가 실제로 정리됐는지 확인합니다. 두 값은 상태 내용 변조까지는 검증하지 못합니다.
- 실패 주입을 해제하는 코드 자체가 예외 때문에 건너뛰지 않도록 어떻게 구성하겠습니까?
  - 모범답변: 실패 설정을 수명 기반 guard로 감싸 소멸자에서 반드시 해제하거나, 모든 catch 경로가 끝난 직후 assertion보다 먼저 해제합니다. 그러면 assertion 실패나 예상 밖 예외를 별도로 처리하는 구조에서도 다음 반복으로 설정이 새지 않습니다.
- 전역 `operator new` 실패 주입은 어떤 테스트 격리 문제를 만들 수 있습니까?
  - 모범답변: 테스트 대상뿐 아니라 assertion, 예외 메시지, iostream, 테스트 프레임워크의 내부 할당까지 실패시킬 수 있습니다. 따라서 단일 프로세스·구간으로 범위를 좁히고 실패 설정 중에는 불필요한 할당을 피하며 매 반복마다 즉시 해제해야 합니다.
...
1. 성공 횟수 계측 후 전 지점 sweep가 경로 누락을 줄이는 방식
   - 모범답변: 동일 입력의 성공 실행이 관찰한 총 clone·할당 횟수를 상한으로 삼고 `1..N`을 모두 실패시키면 현재 구현이 실제로 거치는 각 실패 경계를 검사합니다. 내부 단계 수가 바뀌어도 성공 계측값이 함께 바뀌어 고정 인덱스 테스트보다 누락이 적습니다.
2. "예외가 났다"와 "강한 예외 보장이 지켜졌다" 사이에 필요한 추가 assertion
   - 모범답변: 기대한 예외 형식과 정확한 실패 번호 외에 live count가 기준으로 복원됐는지, source와 target의 크기·출력이 같은지, 필요하면 원소 주소가 같은지, 실패 후 정상 연산이 가능한지를 확인해야 합니다.
3. live object 계수만으로는 상태 변조를 찾을 수 없어 출력·크기도 확인해야 하는 이유
   - 모범답변: 기존 객체를 지우고 같은 수의 다른 객체를 남겨도 live count는 같을 수 있습니다. 크기와 `apply` 결과를 함께 비교해야 누수 없이 발생한 순서 변경·부분 교체·내용 변조를 찾을 수 있습니다.
4. 전역 할당 실패 주입과 타입별 clone 실패 주입의 범위·격리 trade-off
   - 모범답변: 전역 `new` 주입은 문자열과 container를 포함한 모든 할당 경계를 넓게 검사하지만 테스트 기반 코드까지 영향받아 격리가 어렵습니다. 타입별 clone 주입은 결정적이고 원인 추적이 쉽지만 clone 밖의 할당 실패는 다루지 못합니다.
5. Thread 01·02·03·04·11에 같은 검증 패턴을 재사용할 수 있는 이유
   - 모범답변: 모두 여러 준비 연산 뒤 no-throw commit을 수행한다는 공통 구조를 가집니다. 성공 경로 계측, 각 실패점 주입, 자원 기준 복원, 기존 관찰 상태 보존, 실패 후 재사용 확인이라는 같은 assertion 패턴으로 강한 보장을 검증할 수 있습니다.
