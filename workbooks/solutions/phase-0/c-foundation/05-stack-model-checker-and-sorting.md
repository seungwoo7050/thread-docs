...
- 생성기와 checker가 공유해야 하는 것은 명령의 의미입니까, 파싱·출력까지 포함한 전체 코드입니까?
  - 공유 핵심은 `sa`부터 `rrr`까지의 상태 전이입니다. 생성기는 emission을 켜고 checker는 같은 wrapper를 `emit = 0`으로 호출하지만, 입력 parsing과 최상위 판정은 각 프로그램 경계에 남습니다.
- operation 함수가 상태 변경과 명령 출력을 동시에 하면 checker에서 재사용할 때 어떤 문제가 생깁니까?
  - checker가 입력 명령을 재생하면서 다시 명령을 출력할 수 있습니다. 원본은 상태 primitive와 `emit`을 받는 wrapper를 나눠 checker가 조용히 같은 전이를 쓰게 합니다.
- 독립 oracle도 같은 테스트 데이터만 사용하면 놓칠 수 있는 오류는 무엇입니까?
  - 특정 크기·순열·bit 경계에서만 나타나는 오류를 함께 놓칠 수 있습니다. 작은 전체 순열과 여러 고정 seed의 큰 입력, 경계값을 조합해야 합니다.
- 최종 정렬 여부만 확인하는 테스트와 매 operation invariant를 확인하는 테스트는 어떤 버그를 각각 찾습니까?
  - 최종 검사는 명령 stream 전체가 A 정렬·B empty를 만드는지 봅니다. operation 검사는 중간의 value-rank 분리, 원소 유실·중복, size 경계 위반을 바로 특정합니다.
- 이미 정렬된 입력에서 빈 명령 스트림을 기대하는 것은 기능 계약입니까, 성능 계약입니까?
  - 최종 상태만 보면 기능적으로는 다른 유효 stream도 가능하지만, 이 프로젝트는 early exit로 명령 0개를 내므로 명시적인 성능·출력 계약입니다.
...
int	apply_model_command(t_model *model, const char *command)
{
	...
	if (strcmp(command, "sa") == 0)
		return (op_sa(&model->a, 0));
	if (strcmp(command, "sb") == 0)
		return (op_sb(&model->b, 0));
	if (strcmp(command, "ss") == 0)
		return (op_ss(&model->a, &model->b, 0));
	if (strcmp(command, "pa") == 0)
		return (op_pa(&model->a, &model->b, 0));
	if (strcmp(command, "pb") == 0)
		return (op_pb(&model->a, &model->b, 0));
	if (strcmp(command, "ra") == 0)
		return (op_ra(&model->a, 0));
	if (strcmp(command, "rb") == 0)
		return (op_rb(&model->b, 0));
	if (strcmp(command, "rr") == 0)
		return (op_rr(&model->a, &model->b, 0));
	if (strcmp(command, "rra") == 0)
		return (op_rra(&model->a, 0));
	if (strcmp(command, "rrb") == 0)
		return (op_rrb(&model->b, 0));
	if (strcmp(command, "rrr") == 0)
		return (op_rrr(&model->a, &model->b, 0));
	return (0);
}
		...
		const char *const *commands, size_t command_count)
{
	...
	if (model == NULL || (commands == NULL && command_count != 0))
		return (0);
	index = 0;
	while (index < command_count)
	{
		if (!apply_model_command(model, commands[index]))
			return (0);
		index++;
	}
	return (stack_is_complete_sorted(&model->a, &model->b));
}
```
...
1. 제품 코드에서 공유할 경계와 테스트에서 독립시킬 경계.
   - 제품의 generator와 checker는 primitive operation 의미를 공유합니다. 테스트 oracle은 Python list 기반으로 명령을 별도 구현해 C 배열 모델과 같은 버그가 상관되는 위험을 줄입니다.
2. operation의 상태 전이와 명령 emission을 분리한 방법.
   - `stack_swap` 같은 primitive는 상태만 바꾸고, `op_sa` 같은 wrapper가 선택적 `emit`을 처리합니다. checker는 0, generator는 1을 전달합니다.
3. 최종 상태 검증만으로 부족한 invariant.
   - 최종 우연한 정렬만으로는 중간의 원소 유실·중복이나 value-rank pair 분리를 찾기 어렵습니다. 매 전이 후 size 합, multiset, pair 대응, capacity 범위를 검사합니다.
4. 독립 구현이 오히려 유지비를 늘리는 trade-off.
   - 명령 semantics 변경 시 제품과 oracle을 함께 갱신해야 하고 oracle 자체 버그 가능성도 생깁니다. 대신 공유 버그를 독립적으로 발견하는 가치가 있습니다.
5. exhaustive small cases와 seeded large cases를 함께 쓰는 이유.
   - 작은 전체 순열은 모든 국소 배치를 빠짐없이 덮고, 고정 seed 큰 입력은 radix pass·비용 상한과 큰 상태 조합을 재현 가능하게 검증합니다.
...
- `values[i]`와 `ranks[i]`를 항상 함께 이동해야 하는 이유는 무엇입니까?
  - rank는 해당 value의 정렬 key이므로 둘이 떨어지면 생성기는 다른 값을 기준으로 명령을 만듭니다. 원본 테스트는 각 value의 기대 rank를 매 operation 뒤 확인합니다.
- push가 source top을 제거하고 destination top에 넣을 때 배열 이동 비용은 얼마입니까?
  - destination의 기존 size개 pair를 오른쪽으로, source의 남은 size-1개 pair를 왼쪽으로 옮겨 O(destination size + source size)입니다.
- combined operation에서 한쪽이 no-op이어도 다른 쪽은 실행해야 합니까?
  - 예. `ss`, `rr`, `rrr`는 각 primitive를 독립 호출하므로 한 스택에 요소가 부족해도 다른 스택의 유효한 전이는 실행됩니다.
- operation 성공 여부와 명령 출력 성공 여부를 같은 반환값으로 묶으면 어떤 전파 문제가 생길 수 있습니까?
  - 상태는 이미 변경됐지만 emission이 실패해 0을 반환할 수 있습니다. 호출자는 rollback할 수 없으므로 즉시 전체 생성을 실패시키고 이후 명령을 내지 않아야 합니다.
- 배열 표현과 linked-list 표현의 시간·공간·캐시 trade-off는 무엇입니까?
  - index 0 top 배열은 push·rotate가 O(n)이지만 연속 메모리라 locality와 pair 계측이 단순합니다. 연결 리스트는 포인터 재연결이 O(1)일 수 있지만 노드 overhead와 cache miss가 늘어납니다.
...
void	stack_push(t_stack *destination, t_stack *source)
{
	if (destination == NULL || source == NULL || source->size == 0
		|| destination->size >= destination->capacity)
		return ;
	if (destination->size > 0)
	{
		memmove(destination->values + 1, destination->values,
			sizeof(int) * (size_t)destination->size);
		memmove(destination->ranks + 1, destination->ranks,
			sizeof(int) * (size_t)destination->size);
	}
	destination->values[0] = source->values[0];
	destination->ranks[0] = source->ranks[0];
	destination->size++;
	source->size--;
	if (source->size > 0)
	{
		memmove(source->values, source->values + 1,
			sizeof(int) * (size_t)source->size);
		memmove(source->ranks, source->ranks + 1,
			sizeof(int) * (size_t)source->size);
	}
}
...
1. top을 index 0에 둔 선택이 각 operation 비용에 미치는 영향.
   - top 접근과 swap은 O(1)이지만 push는 양쪽 배열을, rotate 계열은 거의 전체 배열을 이동해 O(n)입니다. 원본의 movement metric이 이 물리 비용을 별도로 셉니다.
2. values와 ranks를 구조체 배열 대신 병렬 배열로 둔 trade-off.
   - 정렬 로직이 rank만 연속 접근하기 쉽지만 모든 전이에서 두 배열을 함께 움직여야 하는 invariant 부담이 생깁니다. pair 구조체는 결합은 강하지만 rank-only locality가 달라집니다.
3. no-op 의미가 checker와 combined operation을 단순화하는 방식.
   - 요소 수를 매 wrapper가 분기하지 않고 primitive를 그대로 호출할 수 있습니다. combined operation도 두 primitive를 무조건 실행하면 각쪽의 독립 no-op 의미가 유지됩니다.
4. 상태 전이와 명령 출력 실패를 분리할지 여부.
   - 원본은 primitive에서 상태만 바꾸고 wrapper에서 출력합니다. wrapper 반환값은 emission 실패를 전파하며, 실패 후 변경된 상태를 되돌리기보다 상위 정렬을 중단합니다.
5. invariant 테스트를 최종 정렬 테스트와 별도로 둔 이유.
   - 어느 primitive가 정확히 어떤 pair와 size를 잘못 바꿨는지 국소화하고, 최종 결과에서 우연히 가려질 수 있는 중간 손상을 찾기 위해서입니다.
...
- `INT_MIN`의 절댓값이 `INT_MAX`보다 1 큰 점을 파서에서 어떻게 다룹니까?
  - 부호를 먼저 읽고 음수의 허용 magnitude를 `(unsigned long)INT_MAX + 1`로, 양수는 `INT_MAX`로 둡니다. 정확히 그 음수 한계는 `INT_MIN`으로 저장합니다.
- 값을 누적한 뒤 범위를 검사하는 것보다 digit 추가 전에 검사하는 편이 안전한 이유는 무엇입니까?
  - 이미 `value * 10 + digit`에서 표현 범위를 넘으면 사후 비교 전에 overflow가 발생합니다. `(limit - digit) / 10`과 먼저 비교하면 연산 자체가 안전합니다.
- `-0`과 `+0`은 서로 다른 문자열이지만 왜 중복 값입니까?
  - parser 결과는 둘 다 정수 0이고 중복 계약은 lexical token이 아니라 정수 값에 적용됩니다. 정렬 복사본에서는 인접한 같은 0입니다.
- C whitespace는 어떤 문자 집합으로 한정하며 locale 의존 분류 함수를 쓸지 직접 판정할지 어떻게 결정합니까?
  - 원본은 `' '`, `\t`, `\n`, `\r`, `\v`, `\f` 여섯 ASCII/C whitespace를 직접 판정해 locale과 unsigned-char 호출 규칙에 의존하지 않습니다.
- 정렬된 복사본에서 중복을 찾고 각 원본 값을 binary search하는 방식의 복잡도는 얼마입니까?
  - 정렬 O(n log n), 인접 중복 검사 O(n), n번 binary search O(n log n)로 전체 O(n log n), 추가 공간 O(n)입니다.
- token count가 `INT_MAX`를 넘거나 `count * sizeof(int)`가 overflow하면 언제 실패해야 합니까?
  - allocation이나 int size 저장 전에 count 합산을 검사하고, 배열을 만들기 전에 곱셈 상한을 검사해야 합니다. 부분 allocation 뒤 발견할 이유가 없습니다.
...
static size_t	find_rank(const int *sorted, size_t count, int value)
{
	size_t	low;
	size_t	high;
	size_t	middle;
...
int	parse_int_token(const char *start, size_t length, int *out)
{
	...
	if (start == NULL || out == NULL || length == 0)
		return (0);
	index = 0;
	negative = 0;
	...
	while (index < length)
	{
		if (start[index] < '0' || start[index] > '9')
			return (0);
		digit = start[index] - '0';
		if (magnitude > (limit - (unsigned long)digit) / 10UL)
			return (0);
		magnitude = magnitude * 10UL + (unsigned long)digit;
		index++;
	}
	if (negative && magnitude == (unsigned long)INT_MAX + 1UL)
		*out = INT_MIN;
	else if (negative)
		*out = -(int)magnitude;
	else
...
int	compress_ranks(const int *values, size_t count, int *ranks)
{
	...
	if ((values == NULL && count != 0) || (ranks == NULL && count != 0)
		|| count > (size_t)INT_MAX
		|| count > (size_t)-1 / sizeof(int))
		return (0);
	if (count == 0)
		return (1);
	...
	while (index < count)
	{
		ranks[index] = (int)find_rank(sorted, count, values[index]);
		index++;
	}
	free(sorted);
	return (1);
...
1. 양수·음수의 허용 magnitude가 다른 문제를 해결한 방식.
   - sign을 분리해 양수 limit는 `INT_MAX`, 음수 limit는 `INT_MAX + 1`로 누적했습니다. 최솟값만 별도로 `INT_MIN`에 저장해 양수 int 범위를 거치지 않습니다.
2. token count, 배열 크기, 숫자 누적의 서로 다른 overflow 경계.
   - token count는 최종 stack size의 `INT_MAX`, allocation은 `SIZE_MAX / sizeof(int)`, 숫자는 sign별 magnitude limit를 각각 allocation·연산 전에 검사합니다.
3. hash set 대신 정렬 복사본으로 duplicate와 rank를 함께 해결한 trade-off.
   - O(n) 기대 hash 대신 O(n log n)이지만 추가 자료구조 하나로 인접 duplicate와 정렬 위치 rank를 모두 구하고 해시 정책·충돌 처리가 없습니다. 원본이 이 방식을 씁니다.
4. rank compression이 후속 radix 정렬을 단순화하는 이유.
   - 실제 값의 부호와 간격을 없애고 정확히 0..n-1로 만들어 최대 rank와 필요한 bit 수를 작고 결정적으로 만듭니다.
5. locale 독립 ASCII 문법을 선택한 이유.
   - CLI 계약을 실행 locale과 무관하게 재현하고, `ctype` 계열의 locale 변화와 음수 char 전달 주의를 피할 수 있습니다.
...
- 각 bit pass가 끝났을 때 유지되어야 하는 정렬 invariant는 무엇입니까?
  - 처리한 하위 `bit + 1`개 bit에 대해 A의 원소가 그 key 순서로 stable하게 정렬되고 B는 비어 있어야 합니다.
- pass에서 처음 A에 있던 원소 수만큼만 처리해야 하는 이유는 무엇입니까?
  - 1 bit 원소를 `ra`로 뒤에 다시 넣으므로 현재 size를 조건으로 쓰면 같은 원소를 재처리할 수 있습니다. pass 시작의 `round_size`를 고정해야 각 원소를 정확히 한 번 분류합니다.
- 필요한 bit 수를 최대 rank에서 계산할 수 있는 이유는 무엇입니까?
  - ranks가 빈틈없는 0..n-1이므로 최대값은 `n - 1`이고 그 값을 표현하는 bit 수가 모든 rank를 구분하기에 충분합니다.
- 이미 정렬된 입력에서 아무 명령도 내지 않는 early exit는 어떻게 검증합니까?
  - 정렬된 값을 실행해 stdout이 정확히 빈 bytes인지와 기록된 operation count가 0인지 함께 확인합니다.
- 명령 수만 세면 배열 기반 구현의 실제 이동 비용을 놓칠 수 있는 이유는 무엇입니까?
  - 하나의 `ra`나 `pb`가 index-0 배열에서 O(n) pair 이동을 일으킵니다. 동일 명령 수라도 실행 시 stack size에 따라 memmove 양이 크게 다릅니다.
- wall-clock 대신 명령 수, array movements, peak bytes를 baseline으로 둔 장점은 무엇입니까?
  - 장비·스케줄링 잡음 없이 알고리즘과 표현의 결정적 비용 변화를 비교할 수 있습니다. elapsed time은 참고값으로만 남깁니다.
- 100개·500개 상한 테스트와 다중 고정 seed 동치 검사는 서로 무엇을 보완합니까?
  - 크기별 상한은 평가 기준의 worst-case 예산을 지키는지 보고, 여러 seed는 특정 permutation에 우연히 맞춘 구현이 아닌지와 결과 재현성을 넓게 검사합니다.
...
int	radix_sort(t_stack *a, t_stack *b)
{
	...
	if (a->size < 2 || stack_is_sorted(a))
		return (1);
	bits = 0;
	maximum_rank = a->size - 1;
	while ((maximum_rank >> bits) != 0)
		bits++;
	bit = 0;
	while (bit < bits)
	{
...
1. rank compression이 radix의 bit 수와 구현을 단순화한 방식.
   - rank 범위가 0..n-1이라 signed 값 해석과 값의 큰 간격이 사라지고 `n - 1`의 bit만 처리하면 됩니다.
2. LSD pass의 안정성이 필요한 이유.
   - 새 bit로 0/1을 나눌 때 이전에 정렬한 낮은 bit들의 상대 순서를 보존해야 누적 key 순서가 유지됩니다. rotate와 B에서 다시 push하는 순서가 이를 만듭니다.
3. 작은 입력 전용 전략과 큰 입력 일반 전략의 전환 기준.
   - 원본은 size 2~5를 전용 분기로 처리해 명령 수를 줄이고 6 이상은 예측 가능한 radix를 씁니다. 기준은 정확성 차이가 아니라 command budget입니다.
4. 명령 수와 배열 물리 이동량이 다른 비용 지표인 이유.
   - 외부 평가에서는 한 명령이 한 단위지만 구현 내부에서는 같은 명령이 stack size만큼 pair를 옮길 수 있습니다. 두 지표가 protocol 비용과 자료구조 비용을 각각 보여 줍니다.
5. deterministic seed·exhaustive small permutations·resource baseline의 역할 분담.
   - seed는 큰 입력 동치와 재현성, 작은 전체 순열은 분기 완전성, baseline은 commands·movements·peak bytes의 성능 퇴행을 담당합니다.
...
- `sa` 뒤 EOF는 유효한 마지막 명령인데 빈 입력 뒤 EOF는 왜 EOF입니까?
  - EOF 전 2바이트가 있으면 완성 후보 frame이 있어 vocabulary와 비교할 수 있습니다. 아무 바이트도 없으면 새 frame이 시작되지 않았으므로 stream 종료입니다.
- `sa\0pb\n`을 C 문자열 `"sa"`로 오인하면 어떤 명령 smuggling이 가능합니까?
  - NUL 뒤 bytes를 문자열 비교가 무시해 `sa`를 실행하고 같은 frame의 숨은 `pb`를 검증하지 않을 수 있습니다. reader가 NUL 자체를 즉시 거절해야 합니다.
- 길이 4를 발견한 즉시 나머지 줄을 버리고 계속할지 전체 입력을 오류로 끝낼지 어떻게 정합니까?
  - 이 프로젝트는 4번째 문자를 보는 즉시 checker 전체를 실패시킵니다. malformed frame 뒤 동기화 복구 규칙이 없으므로 계속하지 않습니다.
- 명령 파싱 오류, 입력 I/O 오류, 최종 `KO`는 서로 다른 결과인데 stdout·stderr·exit status를 어떻게 나눕니까?
  - 문법·I/O 오류는 stderr `Error\n`과 nonzero status이고 verdict를 출력하지 않습니다. 정상 stream의 미정렬은 stdout `KO\n`이지만 성공적으로 판정했으므로 status 0입니다.
- checker가 입력 값이 하나도 없을 때 stdin을 읽지 않고 끝내는 계약은 왜 유용합니까?
  - 처리할 모델이 없으므로 pipe 입력을 기다리는 불필요한 block을 피하고, 인자 없는 호출을 조용한 성공으로 일관되게 정의합니다. 원본 main이 먼저 반환합니다.
...
t_read_command	read_command(int fd, t_command *command)
{
	...
	while (1)
	{
		...
		if (read_size == 0 || byte == '\n')
			break ;
		if (byte == '\0' || length >= PS_COMMAND_MAX)
			return (COMMAND_READ_ERROR);
		buffer[length++] = byte;
	}
	if (read_size == 0 && length == 0)
		return (COMMAND_READ_EOF);
	if (length == 0)
		return (COMMAND_READ_ERROR);
	buffer[length] = '\0';
...
1. 최대 문법 길이를 메모리 상한으로 직접 사용한 이유.
   - vocabulary 최장 명령이 3바이트라 그보다 긴 유효 frame은 없습니다. 4바이트 고정 배열이면 NUL sentinel까지 충분하고 공격적인 긴 줄에도 allocation이 늘지 않습니다.
2. EOF가 frame delimiter이기도 하고 stream 종료이기도 한 이중 의미 처리.
   - 누적 length가 양수면 EOF가 마지막 frame을 끝내고, 0이면 새 frame 없는 stream 종료입니다. 같은 read 0을 상태에 따라 다르게 해석합니다.
3. NUL과 overlong 입력을 fail-fast한 이유.
   - 둘 다 현재 frame이 vocabulary에 속할 수 없고 뒤 bytes의 안전한 재동기화 계약도 없습니다. 즉시 오류로 전체 checker를 중단합니다.
4. 문법 오류와 정상 `KO`를 분리한 API·CLI 계약.
   - reader는 문법·I/O를 ERROR로 반환하고 최상위는 stderr와 실패 status를 사용합니다. 모든 명령을 정상 재생한 뒤의 모델 결과만 stdout OK/KO로 냅니다.
5. 한 바이트 read의 단순성과 buffering 최적화 trade-off.
   - frame 상한과 경계 처리가 매우 단순하고 고정 메모리지만 명령 byte마다 syscall이 듭니다. command volume이 커지면 별도 bounded input buffer로 syscall을 줄일 수 있으나 cursor 상태가 추가됩니다.
...
- `malloc`을 전역 macro로 치환하는 방식과 프로젝트 wrapper를 두는 방식의 장단점은 무엇입니까?
  - macro는 기존 호출을 빠르게 가로채지만 헤더 범위·라이브러리 코드·재귀 치환 위험이 큽니다. 명시적 `ps_malloc` wrapper는 call site를 바꿔야 하지만 제품 경계와 fault build 동작이 선명합니다.
- N번째 할당 실패 sweep에서 마지막 실제 할당보다 한 번 뒤의 성공 case도 확인해야 하는 이유는 무엇입니까?
  - sweep이 실제 모든 할당을 셌고 fault가 범위를 벗어나면 정상 경로를 방해하지 않는지 검증합니다. 실패 case만 있으면 off-by-one injector도 통과할 수 있습니다.
- live allocation count만 0이면 invalid free·double free까지 없다고 말할 수 있습니까?
  - 없습니다. 잘못된 포인터 해제나 두 번 해제가 count 구현에 따라 0으로 보일 수 있습니다. allocation metadata·magic 또는 sanitizer로 별도 검출해야 합니다.
- write fault에서 partial prefix가 재출력되지 않았는지 어떻게 검증합니까?
  - 첫 write를 1바이트 short로 만들고 두 번째를 영구 실패시킨 뒤 캡처한 stdout이 정상 stream의 첫 1바이트와 정확히 같은지 확인합니다. 원본 fault test가 이 방식을 씁니다.
- 오류 메시지 자체의 write도 실패할 수 있을 때 원래 실패 status를 어떻게 보존합니까?
  - `write_error()` 결과와 무관하게 이미 정한 nonzero status를 반환합니다. 오류 보고 실패를 성공으로 바꾸거나 재귀적으로 다시 보고하지 않습니다.
- 닫힌 pipe에서 SIGPIPE로 죽지 않고 `EPIPE`를 처리하려면 어느 계층에 정책을 둡니까?
  - 프로그램 초기화에서 SIGPIPE를 무시한 뒤 중앙 `ps_write_all`이 EPIPE를 실패로 전파합니다. 원본 push_swap/checker가 parse·출력 전에 이 정책을 설치합니다.
- sanitizer와 fault injection은 각각 어떤 종류의 결함을 찾습니까?
  - fault injection은 특정 실패 순서의 cleanup·전파·출력 의미를 재현하고, sanitizer는 실제 실행 중 out-of-bounds, use-after-free, double free, UB를 탐지합니다.
...
void	fault_free(t_fault_state *state, void *pointer)
{
	if (pointer == NULL)
		return ;
	/* 최소 harness는 pointer가 이 state의 live allocation이라는 전제다. */
	if (state != NULL && state->live_allocations > 0)
		state->live_allocations--;
	free(pointer);
}
...
ssize_t	fault_read(t_fault_state *state, int fd, void *buffer, size_t count)
{
	...
	state->read_calls++;
	if (state->eintr_read_at != 0
		&& state->read_calls == state->eintr_read_at)
		return (errno = EINTR, -1);
	if (state->fail_read_at != 0
		&& state->read_calls == state->fail_read_at)
		return (errno = EIO, -1);
	return (read(fd, buffer, count));
}
```
...
1. wrapper seam을 제품 코드의 어디에 두었는가.
   - allocation과 read·write가 발생하는 공통 runtime 경계에 `ps_malloc`, `ps_free`, `ps_read`, `ps_write_all`을 두고 parser·stack·checker가 직접 시스템 호출하지 않게 했습니다.
2. 호출 index 의미를 "시도 전" 또는 "시도 후" 중 어떻게 정의했는가.
   - wrapper에 진입할 때 count를 먼저 증가시키고 그 1-based 값이 fault index와 같으면 실제 시스템 호출 없이 실패합니다. 원본 fault build와 같은 의미입니다.
3. fault harness의 관찰 정보가 실제 프로그램 의미를 바꾸지 않게 한 방법.
   - fault 비활성 정상 build는 시스템 함수로 바로 전달하고, 계측·환경 변수 해석은 compile-time fault build에만 둡니다. 보고도 테스트 종료 경계에서 수행합니다.
4. live count, invalid free, double free, peak bytes를 단계적으로 확장하는 방법.
   - allocation 앞에 정렬을 보존하는 header를 두고 size와 magic을 기록합니다. 성공 시 live/current/peak를 갱신하고 free 시 magic으로 invalid·double을 판별한 뒤 current를 감소시킵니다.
5. fault injection과 sanitizer가 상호 대체가 아니라 보완 관계인 이유.
   - injector는 드문 오류 branch를 결정적으로 실행하지만 그 branch의 모든 메모리 위반을 자동 판정하지 않습니다. sanitizer는 실행된 경로의 저수준 위반을 찾지만 원하는 실패 지점을 만들어 주지 않습니다.
