...
- 일반 SHA-256 대신 scrypt 같은 password KDF를 사용해야 하는 이유는 무엇입니까?
  - 꼬리 질문: 존재하지 않는 username에서도 dummy password hash를 검증한 이유는 무엇입니까?
    - 모범답변: 사용자가 없을 때만 KDF를 건너뛰면 응답 시간으로 계정 존재를 추정할 수 있습니다. 고정 dummy 표현에도 같은 scrypt 작업을 수행하고 외부에는 같은 인증 실패를 반환합니다.
  - 꼬리 질문: 저장된 hash 문자열이 손상됐거나 길이가 다를 때 `timingSafeEqual`을 안전하게 사용하려면 어떤 경계를 처리해야 합니까?
    - 모범답변: 고정 prefix·파라미터·hex 길이를 정규식으로 먼저 검증하고 예상 길이의 Buffer만 비교해야 합니다. 형식 손상은 인증 실패 경계에서 처리해 길이 불일치 예외나 내부 표현을 노출하지 않습니다.
...
export async function hashPassword(
  ...
  const key = await new Promise<Buffer>((resolve, reject) => {
    scrypt(password, salt, 64, SCRYPT_OPTIONS,
...
export async function verifyPassword(
  ...
  try {
    ...
    const actual = await new Promise<Buffer>((resolve, reject) => {
      scrypt(password, Buffer.from(match[1], 'hex'), 64, SCRYPT_OPTIONS,
        (error, derived) => error ? reject(error) : resolve(derived));
    });
...
- 일반 hash와 password KDF의 위협 모델 차이
  - 모범답변: 비밀번호는 사람이 고르는 낮은 엔트로피 값이라 DB hash 유출 후 offline 대입이 현실적입니다. scrypt의 메모리·CPU 비용과 사용자별 salt가 각 추측과 사전 계산 비용을 높입니다.
- salt와 pepper의 차이 및 현재 구현 범위
  - 모범답변: salt는 row마다 공개 저장하는 난수로 동일 비밀번호 hash를 다르게 만들고, pepper는 DB 밖에 보관하는 서버 비밀입니다. 현재 원본은 16-byte salt만 사용하며 pepper는 구현하지 않았습니다.
- dummy hash가 막는 정보와 완전히 동일한 timing을 보장하지는 못하는 이유
  - 모범답변: 없는 사용자도 scrypt를 수행해 큰 timing 차이를 줄이지만 DB 조회·scheduler·캐시·런타임 잡음까지 동일하게 만들지는 못합니다. 계정 열거 완화의 한 계층입니다.
- KDF 파라미터 업그레이드와 재해시 전략
  - 모범답변: encoded prefix에 파라미터 version을 남기고 로그인 성공 시 오래된 형식을 새 비용으로 재해시하는 방식이 가능합니다. 현재 정규식은 한 버전만 허용하므로 업그레이드에는 호환 parser와 migration 계획이 필요합니다.
...
- 세션 token 원문을 DB에 저장하지 않고 SHA-256 hash만 저장한 이유는 무엇입니까?
  - 꼬리 질문: 같은 이름의 session cookie가 두 개 있을 때 첫 값이나 마지막 값을 선택하지 않고 요청을 거부한 이유는 무엇입니까?
    - 모범답변: 중복 이름의 선택 규칙은 프록시·parser마다 달라 인증 주체가 모호해질 수 있습니다. 정확히 하나일 때만 받아 request chain 전체가 같은 bearer token을 해석하게 합니다.
  - 꼬리 질문: HttpOnly, SameSite=Lax, Path=/ 속성과 loopback 개발 환경에서 Secure를 생략한 판단을 설명해 보세요.
    - 모범답변: HttpOnly는 JavaScript 노출을 줄이고, SameSite=Lax는 기본 cross-site 전송을 제한하며, Path=/는 발급·삭제 범위를 맞춥니다. 원본은 loopback HTTP 전용이라 Secure를 생략했으며 public production 배포에는 HTTPS와 Secure가 필요합니다.
...
export function sessionTokenFromCookie(
  ...
  const matches = (header ?? '')
    .split(';')
    .map(part => part.trim())
    .filter(part => part.startsWith('wse_session='));
  if (matches.length !== 1) return null;
  const value = matches[0].slice('wse_session='.length);
  // 발급 형식인 32 random bytes의 canonical base64url만 허용한다.
...
- random token과 password의 저장 전략이 다른 이유
  - 모범답변: session token은 32 random bytes의 높은 엔트로피라 빠른 hash여도 offline 추측이 비현실적이지만, 사람이 고른 password는 느린 KDF가 필요합니다.
- 세션 token hash에 느린 KDF가 필수는 아닌 이유
  - 모범답변: 보안은 raw token의 충분한 무작위성에서 나오며 서버는 요청마다 빠르게 lookup해야 합니다. SHA-256은 DB 유출 시 원문 bearer credential의 직접 사용을 막는 저장 경계입니다.
- 중복 cookie를 모호한 입력으로 본 이유
  - 모범답변: 어느 값을 인증과 CSRF에 사용할지 구현 계층마다 달라질 수 있어 request smuggling과 비슷한 해석 차이가 생깁니다. fail-closed로 인증되지 않은 요청으로 처리합니다.
- 개발용 cookie 속성과 production HTTPS 속성의 차이
  - 모범답변: 현재 구현은 loopback HTTP라는 제한된 전제에서 Secure가 없습니다. 인터넷 배포에서는 HTTPS와 Secure를 포함하고 proxy의 scheme 처리까지 검증해야 합니다.
...
- 로그인 시 기존 session 삭제와 새 session 삽입을 한 transaction으로 묶은 이유는 무엇입니까?
  - 꼬리 질문: 새 session INSERT가 실패했는데 기존 session 삭제가 commit되면 사용자 경험과 보안에 어떤 문제가 생깁니까?
    - 모범답변: 로그인은 실패했는데 기존 정상 session만 사라져 사용자가 예기치 않게 로그아웃되고, credential 교체가 부분 적용됩니다. 한 transaction이면 INSERT 실패 시 DELETE도 rollback됩니다.
  - 꼬리 질문: logout을 메모리 상태 초기화만으로 처리하지 않고 DB hash 삭제와 만료 cookie를 함께 수행한 이유는 무엇입니까?
    - 모범답변: DB 삭제는 어느 API 인스턴스에서도 token을 즉시 무효화하고, 만료 cookie는 브라우저가 bearer token을 계속 보내지 않게 합니다. 둘은 서버 권한과 클라이언트 보관이라는 서로 다른 상태를 정리합니다.
  ...
  try {
    ...
    await client.query(
      `INSERT INTO sessions (token_hash, user_id, created_at, expires_at)
       VALUES ($1, $2, $3, $4)`,
      [input.newTokenHash, input.userId, input.createdAt, input.expiresAt],
    );
    await client.query('COMMIT');
    // raw token은 commit 이후에만 성공 결과의 일부가 된다.
    return { rawToken: input.rawToken, expiresAt: input.expiresAt };
  } catch (error) {
...
export async function logoutSession(
  ...
  // 존재하지 않는 row 삭제도 성공이므로 logout은 반복 가능하다.
...
- 세션 회전을 원자적 replace로 본 이유
  - 모범답변: 이전 credential 폐기와 새 credential 발급이 함께 성공하거나 함께 실패해야 한 번의 인증 상태 전이입니다. 별도 autocommit이면 무세션 또는 복수 유효 세션의 중간 상태가 남습니다.
- transaction commit과 HTTP 응답 전달 사이의 ack-loss 문제
  - 모범답변: commit 뒤 응답이 유실되면 서버에는 새 hash가 있지만 브라우저는 새 raw token을 받지 못할 수 있습니다. 원본은 이 구간을 분산 transaction으로 해결하지 않으며 사용자는 다시 로그인해야 할 수 있습니다.
- logout의 server-side revocation과 client-side deletion을 둘 다 하는 이유
  - 모범답변: 서버 삭제만 하면 브라우저가 쓸모없는 secret을 계속 보내고, cookie 삭제만 하면 탈취된 복사본이 DB 만료 전까지 유효합니다. 두 경계를 모두 닫아야 합니다.
- 동시에 여러 로그인 session을 허용할지 한 session만 허용할지의 정책 차이
  - 모범답변: 현재 구현은 요청에 있던 이전 session만 교체하므로 다른 기기 session은 유지합니다. 단일 session 정책이면 사용자 단위 전체 삭제가 필요하지만 모든 기기를 강제 로그아웃시키는 trade-off가 있습니다.
...
- `SELECT monitor WHERE id=?` 뒤 애플리케이션에서 owner를 비교하는 방식보다 `WHERE id=? AND owner_user_id=?`가 나은 이유는 무엇입니까?
  - 꼬리 질문: 다른 사용자의 Monitor와 존재하지 않는 Monitor를 모두 404로 반환한 이유는 무엇입니까?
    - 모범답변: 403과 404를 구분하면 공격자가 UUID를 대입해 다른 사용자의 자원 존재를 확인할 수 있습니다. owner predicate 결과가 없다는 하나의 공개 의미만 반환해 그 단서를 줄입니다.
  - 꼬리 질문: 직접 CheckRun 조회와 Monitor 하위 history 조회 모두에서 owner predicate가 필요한 이유를 설명해 보세요.
    - 모범답변: nested route를 보호해도 `/checks/:id`가 parent owner를 확인하지 않으면 ID를 아는 공격자가 우회합니다. 모든 진입점이 Monitor와 join하거나 owner 조건을 반복해야 합니다.
...
export async function updateMonitorForOwner(
  ...
  const result = await db.query<MonitorRow>(
    `UPDATE monitors
       SET name = $3, url = $4, interval_seconds = $5,
           enabled = $6, updated_at = $7
     WHERE id = $1 AND owner_user_id = $2
     RETURNING *`,
    [monitorId, ownerId, patch.name, patch.url, patch.interval,
      patch.enabled, new Date()],
  );
...
export async function getCheckForOwner(
  ...
  const result = await db.query<CheckRunRow>(
    `SELECT c.*
       FROM check_runs c
       JOIN monitors m ON m.id = c.monitor_id
      WHERE c.id = $1 AND m.owner_user_id = $2`,
    [checkId, ownerId],
  );
  return result.rows[0] ? checkRunFromRow(result.rows[0]) : null;
}
```
...
- SQL predicate 권한 검사와 애플리케이션 후검사의 차이
  - 모범답변: SQL predicate는 허용되지 않은 row를 반환하지 않고 UPDATE/DELETE의 대상 결정과 인가를 한 statement로 묶습니다. 후검사는 민감 row를 먼저 읽고 조회·변경 사이 경합도 만듭니다.
- 404로 존재를 숨기는 정책의 장단점
  - 모범답변: foreign과 absent를 구분하는 enumeration 단서를 줄이지만, 정당한 사용자에게도 "권한 없음" 진단이 덜 명확합니다. 이 프로젝트는 객체 존재 비공개를 우선했습니다.
- nested route와 direct route를 함께 검증해야 하는 이유
  - 모범답변: 동일 자원에 여러 URL이 있으면 가장 약한 경로가 전체 인가 경계가 됩니다. direct CheckRun도 parent Monitor와 join해 owner를 확인해야 우회가 없습니다.
- owner 컬럼 backfill과 NOT NULL 전환 시 필요한 migration 판단
  - 모범답변: 기존 row의 owner를 신뢰할 수 있게 결정한 뒤 FK를 만들고 NOT NULL로 전환해야 합니다. 원본 migration은 지정된 migration owner가 없거나 사용자를 찾지 못하면 진행하지 않아 임의 소유권을 만들지 않습니다.
...
- SameSite cookie만 믿지 않고 session-bound CSRF token과 exact Origin을 함께 검사한 이유는 무엇입니까?
  - 꼬리 질문: CSRF token을 React state나 localStorage에 보관하지 않고 mutation 직전에 읽은 이유는 무엇입니까?
    - 모범답변: session rotation 뒤 stale token 재사용을 막고 장기 browser storage·client prop 노출을 줄이기 위해서입니다. 원본 클라이언트는 `/auth/csrf`에서 받아 해당 mutation의 지역 변수로만 전달합니다.
  - 꼬리 질문: 익명 요청은 401, 인증됐지만 CSRF가 틀린 요청은 403이 되도록 인증을 먼저 수행한 이유는 무엇입니까?
    - 모범답변: 인증 여부가 request의 첫 권한 경계이므로 header 조합과 무관하게 익명 응답을 안정된 401로 만듭니다. 유효 세션이 확인된 뒤에만 Origin·CSRF 정책 위반을 403으로 분류합니다.
...
export function csrfTokenForSession(
  ...
  void secret; // 원본은 32-byte raw session 자체를 HMAC key로 사용한다.
  return createHmac('sha256', rawSession)
    .update('wse-csrf-v1')
    .digest('base64url');
}
...
export function authorizeMutation(
  ...
  if (session === null) throw new ApiError('UNAUTHENTICATED', 'Authentication is required.');
  if (request.method === 'OPTIONS') {
    throw new ApiError('FORBIDDEN', 'Request is not permitted.');
  }
  if (request.method === 'GET' || request.method === 'HEAD') {
    if (request.origin !== undefined && request.origin !== browserOrigin) {
      throw new ApiError('FORBIDDEN', 'Request is not permitted.');
    }
    return;
  }
  if (request.origin !== browserOrigin ||
      typeof request.csrfToken !== 'string' ||
      !/^[A-Za-z0-9_-]{43}$/.test(request.csrfToken)) {
    throw new ApiError('FORBIDDEN', 'Request is not permitted.');
  }
  const expected = csrfTokenForSession(session.rawToken, secret);
  if (!timingSafeEqual(Buffer.from(request.csrfToken), Buffer.from(expected))) {
    throw new ApiError('FORBIDDEN', 'Request is not permitted.');
  }
}
```
...
- SameSite, Origin, CSRF token을 겹쳐 둔 이유
  - 모범답변: SameSite는 cookie 전송 정책, Origin은 요청 출처 증거, CSRF HMAC은 현재 session secret 보유 증거입니다. 서로 다른 설정 오류와 브라우저 경계를 보완하며 어느 하나가 다른 둘을 대체하지 않습니다.
- HMAC token과 random stored token 방식의 trade-off
  - 모범답변: 원본 HMAC은 session token에서 결정적으로 계산해 별도 DB 컬럼이 필요 없고 rotation 시 자동 변경됩니다. random token 저장 방식은 독립 폐기가 가능하지만 저장·조회 lifecycle이 추가됩니다.
- 인증→Origin/CSRF→parse→SQL 순서를 택한 이유
  - 모범답변: 가장 싼 신뢰 경계에서 요청을 먼저 거부해 익명·위조 요청이 body 처리나 저장소·외부 I/O에 도달하지 않게 합니다. 공개 오류 taxonomy도 401과 403으로 결정적입니다.
- CSRF와 XSS의 위협 모델 차이
  - 모범답변: CSRF는 공격 origin이 browser cookie를 자동 전송시키는 위협이고, XSS는 신뢰 origin 안에서 JavaScript를 실행합니다. XSS는 같은 origin의 CSRF endpoint와 mutation을 호출할 수 있어 이 token으로 막는다고 주장할 수 없습니다.
