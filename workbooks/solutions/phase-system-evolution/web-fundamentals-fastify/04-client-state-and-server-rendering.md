...
- 버튼의 `disabled` 속성만으로 중복 submit을 막을 수 없는 실제 반례를 설명해 보세요.
  - 꼬리 질문: `pending`을 React state가 아니라 동기적으로 갱신되는 ref 기반 Set에도 기록한 이유는 무엇입니까?
    - 모범답변: React state 반영과 재렌더 전 같은 tick의 두 handler가 모두 이전 값을 볼 수 있습니다. ref Set은 첫 handler가 첫 `await` 전에 즉시 갱신하므로 두 번째 handler가 동기적으로 거부됩니다.
  - 꼬리 질문: 모든 mutation을 전역 직렬화하지 않고 Monitor별 conflict key를 둔 이유는 무엇입니까?
    - 모범답변: 같은 Monitor의 update·delete·check는 충돌하지만 서로 다른 Monitor 작업은 독립적입니다. `monitor:{id}` granularity는 correctness에 필요한 충돌만 막고 불필요한 직렬화를 피합니다.
...
export class MutationGate {
  ...
  tryEnter(action: MutationAction): AdmissionToken | null {
    ...
    if (this.active.has(key) || this.active.has('logout') ||
        (action.kind === 'logout' && this.active.size > 0)) return null;
    const nonce = Symbol(key);
    this.active.set(key, nonce); // 검사와 등록은 하나의 동기 구간이다.
    return { key, nonce } as AdmissionToken;
  }

  leave(token: AdmissionToken): void {
    // clear 뒤 만들어진 새 작업을 오래된 token이 해제하지 못한다.
    if (this.active.get(token.key) === token.nonce) this.active.delete(token.key);
  }
...
- disabled UI와 correctness gate를 분리한 이유
  - 모범답변: disabled는 사용자 피드백일 뿐 programmatic submit이나 이미 큐에 든 이벤트를 막는 동기화 원시값이 아닙니다. gate가 실제 네트워크 admission을 책임져야 합니다.
- ref/동기 자료구조가 React state보다 admission에 적합한 이유
  - 모범답변: ref 변경은 렌더 batching과 무관하게 즉시 보이며 handler 두 개 사이의 같은 call stack에서도 최신 집합을 읽습니다. React state는 화면 표시용으로 병행합니다.
- conflict key의 granularity를 정한 기준
  - 모범답변: 같은 서버 자원의 상태를 바꾸는 작업은 하나의 Monitor key로 묶고 create와 다른 Monitor는 분리합니다. logout은 account boundary라 모든 key와 양방향으로 충돌합니다.
- 클라이언트 gate와 서버 idempotency가 함께 필요한 이유
  - 모범답변: gate는 한 탭의 우발적 중복만 줄이며 새로고침·다른 탭·응답 유실 재전송을 통제하지 못합니다. durable idempotency는 서버 DB 유일성으로 별도 보장해야 합니다.
...
- logout 뒤 이전 계정의 fetch 응답이 도착해도 새 화면 상태를 덮지 못하게 한 방법을 설명해 보세요.
  - 꼬리 질문: Monitor 삭제 또는 Check 완료 뒤 이전 history 응답이 늦게 도착하는 문제를 어떻게 막았습니까?
    - 모범답변: Monitor별 history version을 올리고 응답 적용 직전에 캡처한 version과 현재 값을 비교합니다. 삭제 시 cache identity도 제거해 이전 read가 삭제된 history를 복원하지 못하게 합니다.
  - 꼬리 질문: Check POST는 commit됐지만 뒤따른 history refresh가 실패한 경우 mutation 자체를 실패로 표시하면 왜 잘못입니까?
    - 모범답변: POST acknowledgement는 실행 의도가 저장됐다는 사실이고 refresh는 별도 GET입니다. GET 실패로 이미 commit된 mutation을 실패로 바꾸면 서버 사실을 거꾸로 표현하고 사용자가 중복 재시도할 수 있습니다.
...
export class AsyncVersionGate {
  ...
  private readonly scopes = new Map<string, { version: number; identity: string }>();

  begin(scope: string, identity: string): RequestToken {
    const version = (this.scopes.get(scope)?.version ?? 0) + 1;
    this.scopes.set(scope, { version, identity });
    return { scope, identity, version, generation: this.generation } as RequestToken;
  }

  isCurrent(token: RequestToken): boolean {
    const current = this.scopes.get(token.scope);
    return token.generation === this.generation &&
      current?.version === token.version && current.identity === token.identity;
  }
...
- AbortController만으로 충분하지 않은 이유
  - 모범답변: abort와 response completion이 경쟁할 수 있고 모든 transport·callback이 취소를 같은 의미로 지키는 것도 아닙니다. token 검사는 결과 적용 시점의 correctness를 보장하고 abort는 자원 절약 최적화로 둘 수 있습니다.
- session generation과 query version을 분리한 이유
  - 모범답변: logout·unmount는 모든 요청을 무효화하지만 한 Monitor history 갱신은 다른 Monitor 요청까지 막을 이유가 없습니다. 전역 수명과 scope별 순서를 다른 축으로 둡니다.
- query identity에 search string을 포함한 이유
  - 모범답변: 같은 Monitor라도 state·limit·cursor가 다르면 다른 서버 질의입니다. URL에서 만든 canonical search를 identity로 사용해야 A 필터 응답이 B 화면에 적용되지 않습니다.
- mutation 상태와 read 상태를 분리한 이유
  - 모범답변: write commit과 후속 read는 별도 요청·실패 경계입니다. 두 상태를 분리해야 "Check 접수 성공, history 새로고침 실패"를 사실대로 동시에 표현할 수 있습니다.
...
- 선택한 Monitor, state filter, limit, cursor를 component local state가 아니라 URL에 둔 이유는 무엇입니까?
  - 꼬리 질문: filter가 바뀔 때 기존 cursor를 제거해야 하는 이유는 무엇입니까?
    - 모범답변: cursor는 monitor·state·limit에 결박된 seek 위치라 조건이 바뀌면 다른 ordered set의 위치가 됩니다. 그대로 보내면 서버가 거부하거나 잘못된 page 경계를 만들 수 있어 제거합니다.
  - 꼬리 질문: `pushState`, `useSearchParams`, fetch cache key가 서로 다른 조건을 가리키지 않게 하려면 어떤 invariant가 필요합니까?
    - 모범답변: 현재 URL에서 정규화한 동일 search string이 렌더 selection, fetch URL, history cache identity의 유일한 입력이어야 합니다. 각 계층에 별도 local 복사본을 두지 않습니다.
...
export function parseHistoryLocation(search: string): HistorySelection {
  ...
  return {
    ...
    state: state === 'SUCCEEDED' || state === 'FAILED' ? state : null,
    limit: rawLimit !== null && /^[0-9]{1,3}$/.test(rawLimit)
      ? Number(rawLimit)
      : null,
    // 최종 형식·조건 검증은 서버 cursor parser가 담당한다.
...
export function updateHistoryLocation(
  ...
  const currentMonitor = next.get('monitor');
  if (change.monitorId !== undefined) {
    if (change.monitorId === null) next.delete('monitor');
    else next.set('monitor', change.monitorId);
    if (change.monitorId !== currentMonitor) next.delete('cursor');
  }
  if (change.state !== undefined) {
    if (change.state === null) next.delete('state');
    else next.set('state', change.state);
    next.delete('cursor');
  }
  if (change.limit !== undefined) {
    if (change.limit === null) next.delete('limit');
    else next.set('limit', String(change.limit));
    next.delete('cursor');
  }
  if (change.cursor !== undefined) {
    if (change.cursor === null) next.delete('cursor');
    else next.set('cursor', change.cursor);
  }
  next.sort();
...
- URL을 탐색 상태의 권위 원천으로 둔 이유
  - 모범답변: direct link·reload·back/forward가 같은 selection을 재구성하고 browser history와 화면이 일치합니다. component state와 URL 사이의 양방향 동기화 문제도 줄어듭니다.
- `pushState`와 router navigation의 trade-off
  - 모범답변: 원본의 native `pushState`는 in-flight mutation과 form draft를 유지하면서 `useSearchParams`를 갱신합니다. full router navigation보다 가볍지만 history 처리와 URL invariant를 코드가 직접 책임집니다.
- query string 전체를 cache identity로 사용한 이유
  - 모범답변: malformed·repeated parameter까지 서버가 최종 판단하도록 원형을 보존하면서, URL이 다른 모든 query를 다른 read로 취급합니다. 일부 필드만 key로 쓰면 조건 충돌이 생길 수 있습니다.
- 클라이언트 검증과 서버 검증의 역할 차이
  - 모범답변: 클라이언트 parser는 표시 편의와 정상 navigation 생성을 담당합니다. 신뢰 경계는 서버이므로 limit·cursor canonicality·owner scope는 매 요청 다시 검증합니다.
...
- 인증된 `/monitors`를 Server Component로 바꾸면서 `headers()`와 `cache: 'no-store'`를 사용한 이유는 무엇입니까?
  - 꼬리 질문: Monitor collection을 읽은 뒤 selected history를 읽는 사이 session이 사라지면 두 결과를 함께 버리고 login으로 보낸 이유는 무엇입니까?
    - 모범답변: 두 read는 하나의 초기 private 화면 payload입니다. 두 번째가 401이면 첫 번째 collection도 더 이상 유효한 현재 session snapshot이 아니므로 혼합 상태를 렌더하지 않고 전부 폐기합니다.
  - 꼬리 질문: login/logout에서 client router 전환보다 document replacement를 선택한 이유를 account-boundary cache 관점에서 설명해 보세요.
    - 모범답변: document replacement는 이전 React state와 route/RSC client cache를 함께 버립니다. 비용은 전체 navigation이지만 다른 계정의 private payload가 client cache에서 재사용될 경계를 단순하게 닫습니다.
...
export async function loadInitialWorkspace(
  ...
  // raw Cookie의 중복·모호성 판단은 권위자인 Fastify에 그대로 맡긴다.
  for (const name of ['cookie', 'origin']) {
    const value = requestHeaders.get(name);
    if (value !== null) forwarded.set(name, value);
  }
  const apiOrigin = process.env.API_ORIGIN ?? 'http://127.0.0.1:4312';
  const read = async <T>(path: string): Promise<T> => responseData<T>(await fetch(
  ...
  try {
    ...
    const initial: InitialWorkspace = {
      authenticated: true, monitors, histories: {}, error: null,
    };
    if (selectedQuery !== null) {
      const parameters = new URLSearchParams(selectedQuery);
      const { monitorId, cursor, state, limit } = parseHistoryLocation(parameters.toString());
      if (monitorId) {
        ...
        const data = await read<CheckHistoryPage>(
          `/monitors/${encodeURIComponent(monitorId)}/checks${search ? `?${search}` : ''}`,
        );
        initial.histories[monitorId] = { search, status: 'success', data, error: null };
      }
    }
    return initial;
  } catch (failure) {
    if (failureCode(failure) === 'UNAUTHENTICATED') return { redirect: '/login' };
    throw failure;
  }
}
```
...
- Server Component와 Client Component 경계를 나눈 기준
  - 모범답변: request cookie가 필요한 private 초기 read는 Server Component 계층이 소유하고, event handler·form draft·URL 상호작용은 작은 client island가 소유합니다. client에는 공개 API payload만 넘깁니다.
- `no-store`와 요청 시 header 접근이 필요한 이유
  - 모범답변: `headers()`는 현재 request의 인증 문맥을 읽게 하고 `no-store`는 사용자별 API 결과가 shared fetch cache에 남지 않게 합니다. 둘이 private HTML/RSC의 request scope를 고정합니다.
- 여러 server read를 하나의 화면 단위로 취급한 이유
  - 모범답변: collection과 selected history가 함께 hydration되는 초기 상태이므로 session 세대가 섞이면 안 됩니다. 인증 상실 시 모두 폐기하고, 비인증 오류만 history query 상태로 분리합니다.
- document replacement의 비용과 account cache 안전성 이점
  - 모범답변: 전체 문서와 asset 경계를 다시 거쳐 client navigation보다 비싸지만 메모리 state·RSC route cache·진행 중 callback을 한 번에 제거해 account 전환 invariant가 단순해집니다.
