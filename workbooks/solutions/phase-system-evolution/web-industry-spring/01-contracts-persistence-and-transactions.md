...
- 전송 형식 검증과 도메인 값 검증을 한 단계에서 처리할 때 생기는 장단점은 무엇입니까?
  - 모범답변: 한 경계에서 JSON 종류와 값 범위를 모두 확인하면 저장 전에 하나의 400으로 수렴하고 호출 순서가 단순합니다. 반면 transport와 domain 규칙이 결합되어 재사용성과 오류 세분화가 낮아지는 비용이 있습니다.
- 변환 중 `ArithmeticException`과 `NumberFormatException`을 모두 고려해야 하는 이유는 무엇입니까?
  - 모범답변: `intValueExact()`는 소수부나 int 범위 초과를 `ArithmeticException`으로 알릴 수 있고, 비정상 숫자 표현 변환은 `NumberFormatException`으로 나타날 수 있습니다. 둘 다 공개적으로는 같은 입력 오류입니다.
- 잘못된 요청이 저장소를 한 번도 변경하지 않았음을 어떻게 검증하겠습니까?
  - 모범답변: 각 malformed fixture에 대해 400을 확인하고 저장소를 spy로 두어 create/replace 호출이 0회인지, 통합 테스트에서는 전후 row snapshot과 count가 같은지 확인합니다.
- JSON 문서 뒤에 두 번째 토큰이 붙는 경우를 필드 검증만으로 막을 수 있습니까?
  - 모범답변: 이미 만들어진 단일 `JsonNode`만 보는 함수로는 문서 뒤 토큰을 알 수 없습니다. 원본은 `spring.jackson.deserialization.fail-on-trailing-tokens=true`로 parser 경계에서 거부합니다.
...
static CreateMonitor fromJson(JsonNode body) {
    if (body == null || !body.isObject()
            ...
            || !body.path("url").isTextual()
            || !body.path("interval").isNumber()
            || !body.path("enabled").isBoolean()) {
        throw invalidInput();
    }
    String name = body.get("name").textValue().strip();
    if (name.isEmpty() || name.length() > 100 || name.indexOf('\0') >= 0) {
        throw invalidInput();
    }
    int interval;
    try {
        // 60.0은 수학적 정수 60이지만 60.5와 범위 초과는 정확 변환에서 거부된다.
        interval = body.get("interval").decimalValue().intValueExact();
    } catch (ArithmeticException | NumberFormatException error) {
        throw invalidInput();
    }
    if (interval < 1 || interval > 86_400) throw invalidInput();
...
static ResponseStatusException invalidInput() {
    return new ResponseStatusException(HttpStatus.BAD_REQUEST, "Request input is invalid.");
}
```
...
1. 자동 DTO 바인딩보다 명시적 노드 검증을 택한 이유와 그 비용
   - 모범답변: Jackson coercion 전에 실제 JSON kind를 확인해 문자열 숫자·boolean을 계약 위반으로 거부합니다. 대신 필드 검증 코드가 길고 DTO binding의 선언적 편의가 줄어듭니다.
2. 전송 형식 제약과 도메인 값 제약을 나눈 기준
   - 모범답변: `isNumber` 같은 검사는 wire 표현이고, exact int 및 1~86,400 검사는 저장 가능한 도메인 값입니다. 한 함수에 있더라도 판단 순서를 분리해 설명할 수 있습니다.
3. exact conversion으로 반올림·오버플로를 막은 방식
   - 모범답변: `decimalValue().intValueExact()`를 사용해 소수부가 0인 값만 받고 int 범위를 벗어나면 예외로 거부합니다. `double` 변환이나 cast를 거치지 않습니다.
4. 잘못된 입력에서 상태 변화가 없도록 만든 호출 순서
   - 모범답변: `CreateMonitor.fromJson`과 URL canonicalization이 모두 끝난 뒤에만 `store.create/replace`를 호출하므로 invalid 경로에는 repository 호출이 없습니다.
5. trailing-token 검사는 함수 바깥의 파서 설정이 담당하는 이유
   - 모범답변: 함수가 받는 시점에는 하나의 syntax tree만 남아 뒤 토큰 정보를 잃었습니다. parser가 문서 전체 소비를 책임지는 계층에서 추가 토큰을 거부해야 합니다.
...
- HTTP 200이지만 성공 payload가 깨져 있다면 UI는 성공으로 처리해야 합니까?
  - 모범답변: 아니요. status와 `{data}` shape, payload validator가 모두 통과해야 성공이며, 그렇지 않으면 기존 화면·캐시를 유지한 채 `INTERNAL_ERROR`로 fail-closed 합니다.
- 오류 메시지 문구 대신 상태 코드와 오류 코드를 기준으로 분기해야 하는 이유는 무엇입니까?
  - 모범답변: 문구는 번역·개선으로 바뀌고 민감한 내부 세부를 포함할 수 있지만 code/status 조합은 안정된 wire contract입니다. UI 로직과 표시 문구를 분리할 수 있습니다.
- 네트워크 실패, 비정상 JSON, 잘못된 오류 봉투를 어떻게 같은 안전 범주로 수렴시키겠습니까?
  - 모범답변: transport 호출부와 `response.json()`·envelope validation의 알 수 없는 실패를 모두 공개 세부 없는 `INTERNAL_ERROR`로 변환하고 캐시 mutation을 하지 않습니다.
- 서버 로그에 남길 정보와 클라이언트에 공개할 정보를 어떻게 나누겠습니까?
  - 모범답변: 서버는 requestId와 허용된 route·status·내부 원인 code로 진단하고, 클라이언트에는 안정된 오류 code와 일반 문구만 보냅니다. credential·SQL·stack·원문 예외는 공개하지 않습니다.
...
async function readData<T>(
  ...
  const statuses: Record<ApiErrorCode, number> = {
    INVALID_INPUT: 400, NOT_FOUND: 404, UNAUTHENTICATED: 401,
    FORBIDDEN: 403, CONFLICT: 409, INTERNAL_ERROR: 500,
  };
  const failure = (code: ApiErrorCode, status: number): never => {
    throw Object.assign(new Error(code), { code, status } satisfies ApiFailure);
  };

  let body: unknown;
  try { body = await response.json(); }
  catch { return failure('INTERNAL_ERROR', 500); }
  if (!response.ok) {
    if (object(body) && Object.keys(body).length === 1 && object(body.error)
        && typeof body.error.message === 'string') {
      const code = body.error.code;
      if (typeof code === 'string' && code in statuses
          && statuses[code as ApiErrorCode] === response.status) {
        return failure(code as ApiErrorCode, response.status);
      }
    }
    return failure('INTERNAL_ERROR', 500);
  }
  if (!object(body) || Object.keys(body).length !== 1
      || !Object.hasOwn(body, 'data') || !valid(body.data)) {
    return failure('INTERNAL_ERROR', 500);
  }
  return body.data;
}
...
1. TypeScript 정적 타입이 네트워크 신뢰 경계를 넘지 못하는 이유
   - 모범답변: generic과 interface는 컴파일 후 사라지고 proxy·서버 버그·오래된 배포가 다른 JSON을 보낼 수 있습니다. 실제 `unknown`을 runtime guard로 다시 확인해야 합니다.
2. 성공 상태와 payload 모양을 함께 검증하는 방식
   - 모범답변: `response.ok`, 최상위 key가 오직 `data`인지, 그리고 호출자가 준 type guard가 참인지 모두 확인합니다. 어느 하나라도 어긋나면 성공으로 적용하지 않습니다.
3. 오류 문구 대신 코드에 의존하는 호환성 이점
   - 모범답변: 표시 문구를 바꿔도 동작 분기가 유지되고, 클라이언트가 서버 내부 예외 텍스트를 파싱하는 취약한 결합을 피합니다.
4. 서버 내부 예외와 공개 오류를 분리한 기준
   - 모범답변: 사용자가 고칠 수 있는 안정된 범주만 code로 공개하고 예상하지 못한 원인·stack·SQL은 서버 관측에만 남깁니다.
5. fail-open보다 fail-closed를 택한 이유
   - 모범답변: 깨진 성공 payload를 부분 적용하면 cache와 UI가 서버 사실과 어긋납니다. 기존 정상 상태를 유지하고 다시 읽을 수 있게 하는 편이 안전합니다.
...
- 애플리케이션이 나노초 시각을 응답한 뒤 DB가 마이크로초로 잘라 저장하면 어떤 문제가 생깁니까?
  - 모범답변: create 응답과 재조회 값이 달라 equality·cache 비교·cursor 경계가 불안정해집니다. 첫 응답 전에 `ChronoUnit.MICROS`로 정규화해야 round trip이 같습니다.
- 데이터베이스 세션 시간대가 `Asia/Seoul`이어도 같은 `Instant`를 복원하려면 무엇을 확인해야 합니까?
  - 모범답변: 컬럼이 `timestamp with time zone`이고 JDBC/Hibernate가 절대 시각으로 매핑되는지 확인해야 합니다. 표시 timezone과 저장 instant 의미를 혼동하지 않고 JVM 기본 timezone을 바꾼 round-trip 테스트를 합니다.
- 컬럼이 빠졌거나, ORM이 모르는 필수 컬럼이 추가된 상태에서 서버를 띄우면 어떤 실패 방식이 바람직합니까?
  - 모범답변: 첫 요청의 임의 지점에서 SQL 오류가 나기보다 시작 시 schema compatibility 검사를 실패시켜 readiness 전에 프로세스를 준비 불가로 만들어야 합니다.
- `0`과 `null`을 구분하지 못하는 매핑 버그를 어떻게 찾겠습니까?
  - 모범답변: `latencyMs=0`, `httpStatus=null`, `enabled=false` fixture를 저장·flush·clear·재조회하고 첫 API 응답과 비교합니다. primitive 기본값이나 truthy 보정을 쓰지 않았는지도 확인합니다.
...
final class CheckRunEntity {
    static CheckRunEntity fromDomain(CheckRun run) {
        ...
        row.id = run.id();
        row.monitorId = run.monitorId();
        row.state = run.state();
        row.httpStatus = run.httpStatus();
        row.latencyMs = run.latencyMs();
        row.failureReason = run.failureReason();
        row.startedAt = run.startedAt() == null ? null
                : run.startedAt().truncatedTo(ChronoUnit.MICROS);
        row.finishedAt = run.finishedAt() == null ? null
                : run.finishedAt().truncatedTo(ChronoUnit.MICROS);
        return row;
    }

    CheckRun toDomain() {
        var run = new CheckRun(id, monitorId, state, httpStatus, latencyMs,
                failureReason, startedAt, finishedAt);
        requireValid(run);
        return run;
    }

    private static void requireValid(CheckRun run) {
        boolean noResult = run.httpStatus() == null && run.latencyMs() == null
                && run.failureReason() == null;
        boolean valid = switch (run.state()) {
            case "QUEUED" -> run.startedAt() == null && run.finishedAt() == null && noResult;
            case "RUNNING" -> run.startedAt() != null && run.finishedAt() == null && noResult;
            case "ABORTED" -> run.startedAt() != null && run.finishedAt() != null && noResult;
            case "SUCCEEDED" -> run.startedAt() != null && run.finishedAt() != null
                    && run.httpStatus() != null && run.httpStatus() >= 200 && run.httpStatus() < 300
                    && run.latencyMs() != null && run.latencyMs() >= 0 && run.failureReason() == null;
            case "FAILED" -> run.startedAt() != null && run.finishedAt() != null
                    && run.latencyMs() != null && run.latencyMs() >= 0
                    && (("HTTP_STATUS".equals(run.failureReason()) && run.httpStatus() != null
                         && (run.httpStatus() < 200 || run.httpStatus() >= 300))
                        || (("TIMEOUT".equals(run.failureReason())
                             || "CONNECTION_FAILURE".equals(run.failureReason()))
                            && run.httpStatus() == null));
            default -> false;
        };
        if (!valid || (run.startedAt() != null && run.finishedAt() != null
                && run.finishedAt().isBefore(run.startedAt()))) {
            throw new IllegalArgumentException("Inconsistent CheckRun state");
        }
    }
}
```
...
1. 첫 응답부터 DB 정밀도로 정규화해야 하는 이유
   - 모범답변: 저장 직후 응답과 재시작 후 재조회가 같은 값을 가져야 cache·cursor·동등성 판단이 안정적입니다. DB가 나중에 자를 정밀도를 애플리케이션이 먼저 적용합니다.
2. 엔티티를 API 모델로 직접 노출하지 않은 이유
   - 모범답변: JPA lifecycle·내부 owner/lease 컬럼과 공개 계약을 분리하고 lazy proxy나 새 컬럼이 우연히 직렬화되는 것을 막습니다.
3. nullable wrapper 타입을 사용한 필드와 primitive를 사용한 필드의 기준
   - 모범답변: 아직 관찰되지 않은 status·latency는 null 자체가 상태 의미이므로 `Integer/Long`이 필요합니다. 항상 존재하는 값만 primitive로 두어 0과 미정 상태를 섞지 않습니다.
4. 시간대와 시각 정밀도를 별개 문제로 본 이유
   - 모범답변: timezone은 같은 instant의 표현 문제이고 precision은 저장 가능한 fraction의 문제입니다. UTC instant를 유지해도 나노초/마이크로초 차이는 별도로 정규화해야 합니다.
5. 스키마 호환성 검사를 애플리케이션 준비 완료 전에 수행하는 이유
   - 모범답변: 호환되지 않는 mapping으로 일부 요청이 잘못 쓰이기 전에 배포를 실패시키고, 트래픽을 받지 않는 명확한 운영 상태로 만들기 위해서입니다.
...
- 외부 I/O 중 트랜잭션과 DB 커넥션을 오래 잡으면 어떤 자원 고갈과 잠금 문제가 생깁니까?
  - 모범답변: 느린 endpoint 수만큼 pool connection이 고갈되고 transaction snapshot·row lock이 오래 유지되어 다른 요청의 대기와 vacuum 지연을 키울 수 있습니다.
- Spring 프록시 기반 트랜잭션에서 같은 클래스의 메서드를 내부 호출하면 어떤 함정이 있습니까?
  - 모범답변: `this.method()`는 Spring proxy를 통과하지 않아 별도 `@Transactional` 전파·readOnly 설정이 적용되지 않을 수 있습니다. transaction 역할을 다른 bean/interface 경계로 분리해야 합니다.
- 외부 호출은 성공했지만 결과 저장이 실패하면 어떤 의미론이 남습니까?
  - 모범답변: endpoint 관찰은 실제로 발생했지만 권위 DB에는 결과가 없어 클라이언트에 성공을 확정할 수 없습니다. 이 구조만으로는 그 partial failure를 원자적으로 없앨 수 없습니다.
- 이 경계는 Thread 09의 영속 큐로 어떻게 발전했습니까?
  - 모범답변: API가 먼저 QUEUED intent와 ID를 commit해 202를 반환하고 worker가 claim·실행·terminal 저장을 이어받습니다. crash 뒤에도 DB 상태와 lease로 복구할 근거가 생깁니다.
...
final class CheckService {
    CheckRun execute(UUID ownerId, UUID monitorId) {
        // 각 호출의 @Transactional은 MonitorTransactions 구현 proxy에서 끝난다.
        MonitorTarget target = transactions.loadAuthorizedTarget(ownerId, monitorId);
        CheckRun observed = checker.observe(monitorId, target.url());
        // 삭제·소유권 변경 경합은 쓰기 경계에서 다시 확인한다.
        return transactions.saveObservedResult(ownerId, observed);
    }
}
```
...
1. 트랜잭션을 외부 I/O 바깥으로 밀어낸 자원·잠금상의 이유
   - 모범답변: latency 상한이 다른 네트워크를 기다리는 동안 pool connection·snapshot·lock을 보유하지 않아 DB 처리량과 격리를 보호합니다.
2. 읽기와 쓰기를 한 트랜잭션으로 묶지 않은 trade-off
   - 모범답변: connection 점유는 짧아지지만 조회 뒤 Monitor 삭제·변경이나 외부 성공 뒤 저장 실패가 가능합니다. 저장 시 owner/parent를 다시 확인해 데이터 invariant만 지킵니다.
3. 프록시 기반 트랜잭션의 self-invocation 함정
   - 모범답변: 같은 객체 내부 호출은 proxy advice를 우회하므로 기대한 새 transaction이 생기지 않습니다. 별도 transaction bean을 주입해 외부 호출로 경계를 통과시킵니다.
4. 외부 성공 후 저장 실패를 완전히 없애지 못하는 이유
   - 모범답변: PostgreSQL transaction과 원격 HTTP side effect를 하나의 원자적 commit으로 묶는 프로토콜이 없기 때문입니다. 예외를 성공으로 바꾸지 않고 불확실성을 보존해야 합니다.
5. 그 빈틈을 영속 큐와 상태 머신으로 개선하는 방법
   - 모범답변: 외부 호출 전에 durable intent를 만들고 claim owner·lease·terminal CAS를 기록하면 실행 진행과 crash를 재구성할 수 있습니다. 원격 호출 자체의 exactly-once까지 보장하는 것은 아닙니다.
