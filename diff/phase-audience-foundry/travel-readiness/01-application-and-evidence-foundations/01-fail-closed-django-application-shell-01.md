...
## `docs: reconcile Admin session boundary`

diff --git a/docs/IMPLEMENTATION-PLAN.md b/docs/IMPLEMENTATION-PLAN.md
index eca1d6d..e22cec9 100644
--- a/docs/IMPLEMENTATION-PLAN.md
+++ b/docs/IMPLEMENTATION-PLAN.md
@@ -288,11 +288,11 @@ PRG와 무보존 요구를 함께 만족하는 설계가 별도 atom에서 검
 
 ### 13. `build(web): add the public web boundary`
 
-- 질문: public app이 session/cache 없이 server-rendered read path만 갖도록 격리되는가?
+- 질문: public app이 session을 읽거나 쓰지 않고 server-rendered read path만 갖도록 격리되는가?
 - 목적: publication query와 template route를 다음 atom에서 추가할 최소 app boundary를 만듭니다.
 - 의존성: atom 3과 12
 - 예상 파일: `public_web/__init__.py`, `public_web/apps.py`, `travel_readiness/settings.py`
-- 검증: installed app check, session middleware/cache backend 부재, privacy config scan
+- 검증: installed app check, public session access 부재, `DummyCache`, privacy config scan
 - rollback: public app registration과 package를 제거; publication history 유지
 
 ### 14. `feat(web): render the independent entry card`


## `build: add minimal Django project shell`

diff --git a/manage.py b/manage.py
new file mode 100644
index 0000000..e23cc99
--- /dev/null
+++ b/manage.py
@@ -0,0 +1,14 @@
+#!/usr/bin/env python
...
+    main()
diff --git a/travel_readiness/__init__.py b/travel_readiness/__init__.py
new file mode 100644
index 0000000..6152cbf
--- /dev/null
+++ b/travel_readiness/__init__.py
@@ -0,0 +1 @@
+"""Travel readiness Django project."""
diff --git a/travel_readiness/settings.py b/travel_readiness/settings.py
new file mode 100644
index 0000000..6996bc3
--- /dev/null
...
+]
+
+ROOT_URLCONF = "travel_readiness.urls"
+TEMPLATES = [
+    {
...
+    }
+]
+WSGI_APPLICATION = "travel_readiness.wsgi.application"
+
+DATABASES = {
+    "default": {
