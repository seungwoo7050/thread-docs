...
+          <select id="regions-date" name="date"{% if date_error %} aria-invalid="true" aria-describedby="validation-title"{% endif %}>
             {% for option in date_options %}
               <option value="{{ option.value }}"{% if option.selected %} selected{% endif %}>{{ option.label }}</option>
             {% endfor %}
@@ -38,7 +38,7 @@
...
+                    <div><dt>최저 조사값</dt><dd>{% if row.minimum_machine %}<data value="{{ row.minimum_machine }}">{% endif %}{{ row.minimum_label }}{% if row.minimum_machine %}</data>{% endif %}</dd></div>
+                    <div><dt>최고 조사값</dt><dd>{% if row.maximum_machine %}<data value="{{ row.maximum_machine }}">{% endif %}{{ row.maximum_label }}{% if row.maximum_machine %}</data>{% endif %}</dd></div>
                   </dl>
                   {% if row.meter %}
