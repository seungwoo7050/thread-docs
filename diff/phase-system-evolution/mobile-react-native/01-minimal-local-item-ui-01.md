...
## `feat: establish in-memory React Native Item UI`

diff --git a/.gitignore b/.gitignore
new file mode 100644
index 0000000..edc72f1
--- /dev/null
+++ b/.gitignore
@@ -0,0 +1,11 @@
+node_modules/
...
new file mode 100644
index 0000000..91e31c1
--- /dev/null
+++ b/TRACK.md
@@ -0,0 +1,40 @@
+# Offline Item Tracker — React Native
+
...
+counter without synchronization meaning. Nothing is persisted, and a cold process
...
+
+Verification records are under `verification/`.
diff --git a/__tests__/App.test.tsx b/__tests__/App.test.tsx
new file mode 100644
index 0000000..119a013
--- /dev/null
...
+
+  items = itemsReducer(items, {type: 'create', id: 'item-001', title: 'Alpha', now: now()});
+  const firstId = items[0].id;
+  items = itemsReducer(items, {type: 'create', id: 'item-002', title: 'Beta', now: now()});
+  expect(items.map(item => item.title)).toEqual(['Alpha', 'Beta']);
+  items = itemsReducer(items, {type: 'rename', id: firstId, title: 'Alpha edited', now: now()});
+  items = itemsReducer(items, {type: 'toggle', id: firstId, now: now()});
+  expect(items[0].completed).toBe(true);
+  items = itemsReducer(items, {type: 'toggle', id: firstId, now: now()});
+  expect(items[0].completed).toBe(false);
+  items = itemsReducer(items, {type: 'toggle', id: firstId, now: now()});
+  items = itemsReducer(items, {type: 'delete', id: 'item-002'});
+
+  expect(items).toEqual([{id: firstId, title: 'Alpha edited', completed: true, version: 5, updatedAt: 1700000005000}]);
+  expect(firstId).toBe('item-001');
+});
+
+test('blank titles cannot create or erase an Item', () => {
+  expect(itemsReducer([], {type: 'create', id: 'item-001', title: ' ', now: 1700000000000})).toEqual([]);
...
+
+react {
+    // Every APK includes JS; device verification does not require a Metro server.
+    debuggableVariants = []
...
new file mode 100644
index 0000000..6bdc77a
--- /dev/null
+++ b/app.json
@@ -0,0 +1 @@
+{"name":"OfflineItemTracker","displayName":"Offline Item Tracker"}
diff --git a/babel.config.js b/babel.config.js
new file mode 100644
index 0000000..09b5f09
--- /dev/null
+++ b/babel.config.js
@@ -0,0 +1 @@
...
+      "version": "0.76.9",
+      "resolved": "https://registry.npmjs.org/@react-native/metro-config/-/metro-config-0.76.9.tgz",
...
+      "version": "0.76.9",
+      "resolved": "https://registry.npmjs.org/@react-native/typescript-config/-/typescript-config-0.76.9.tgz",
+      "integrity": "sha512-68xGswpZOrCvDd1Wu6H7ZdluIDmNbN0Uq8RVnm+IQMnYx90fVHL+iNW4hClgoY/TIcsWnQQL6shES4n/1kz/fg==",
+      "dev": true,
+      "license": "MIT"
+    },
...
+
+export function itemsReducer(items: Item[], action: ItemAction): Item[] {
+  if (action.type === 'create') {
+    const title = action.title.trim();
+    if (!title || items.some(item => item.id === action.id)) {
+      return items;
+    }
+    return [...items, {id: action.id, title, completed: false, version: 1, updatedAt: action.now}];
+  }
...
+        return item;
+      }
+      return {...item, title, version: item.version + 1, updatedAt: action.now};
+    }
+    return {...item, completed: !item.completed, version: item.version + 1, updatedAt: action.now};
+  });
+}
diff --git a/tsconfig.json b/tsconfig.json
new file mode 100644
index 0000000..20c726d
--- /dev/null
+++ b/tsconfig.json
@@ -0,0 +1,4 @@
+{
